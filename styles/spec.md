...
Exemplars in this file and their provenance:
  - RFC 2119   - IETF, S. Bradner 1997 - freely redistributable ("Distribution of this memo is unlimited")
  - RFC 896    - IETF, J. Nagle 1984    - pre-2008 IETF material; verbatim redistribution OK, no modification. Page headers/whitespace stripped only.
  - RFC 9110   - IETF/IESG 2022         - BCP 78 / Revised BSD for code components; excerpt only
  - SQLite file format §1.2 - sqlite.org - Public domain
-->

# Spec writing guide

<!-- RUBRIC - what an agent should reproduce from these exemplars
1. State the problem and quantify it before proposing anything (RFC 896: "41 byte
   packets... this 4000% overhead"). Evidence precedes mechanism.
2. Use RFC 2119 keywords deliberately: MUST/SHOULD/MAY carry different force, and
   per §6 are reserved for interoperability or harm-prevention - not style.
3. Be exact about observable facts: byte offsets, bounds, enumerated cases
   (SQLite: page size "a power of two between 512 and 65536 inclusive").
4. Give each requirement a rationale a reader can weigh (RFC 9110 pairs each
   MUST/SHOULD with why - e.g. why a TRACE request must not carry credentials).
Anti-patterns: MUST everywhere; requirements with no stated reason; prose that
restates a mechanism without saying what problem it solves.
-->

Specifications define correctness. They do not prescribe implementation order. Implementation order changes, but requirements don't. Organize documents by concept rather than shared infrastructure. Split documents when they describe different correctness properties.

Define each concept exactly once. Refer to it elsewhere rather than repeating or redefining it.

**BAD** - Projected coordinates have not been defined:

```text
The frame enters projected coordinates...
```
...
## RFC 2119 (**stick to these guidelines**)

<!-- Annotation: This is the normative-language contract, included as rules to
FOLLOW, not prose to imitate. Note §3/§5: SHOULD leaves room to deviate "with
full implications understood"; MAY is truly optional but still demands
interoperability. §6 is the guardrail against keyword inflation. -->
...
<!-- Annotation: The canonical problem -> evidence -> solution -> caveats arc. Nagle names
the failure ("congestion collapse"), shows it happening on a real network,
quantifies the cost (41-byte packets, 4000% overhead), rejects the obvious fix
(a fixed timer) with a reason, then proposes the adaptive one. Note it explicitly
says it "does not specify any standards" - it argues, it doesn't decree. -->
...
<!-- Annotation: Modern normative language doing real work. Every MUST NOT / SHOULD
here is attached to a reason (a TRACE request "MUST NOT generate fields
containing sensitive data" because the response reflects them back). This is the
model for requirement + rationale in one breath. -->
...
## SQLite (file format §1.2)

<!-- Annotation: Precision as a discipline. Exact bounds ("a power of two between
512 and 65536 inclusive"), the exact byte offset of the page-size field, and an
exhaustive enumeration of every possible page type. A format spec leaves nothing
to inference; contrast with prose that says "the page size is configurable." -->
