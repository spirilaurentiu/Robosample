    ...
    SimTK::Real collisionFrequency)
{
	...
	for (const auto& flex_list : flexibilities) {
		for (const auto& topology : topologies) {
			...
			const auto rigidBodyIndices = omm.getRigidBodyIndices(atoms.size(), topology.getBonds(), flex_list);
			...
			for (const auto& bond : topology.getBonds()) {
				...
				const auto aIx1 = atoms[globalIndex1].identity.compoundAtomIndex;
				const auto aIx2 = atoms[globalIndex2].identity.compoundAtomIndex;
			...
			for (const auto& angle : topology.getAngles()) {
				...
				const auto aIx1 = atoms[globalIndex1].identity.compoundAtomIndex;
				const auto aIx2 = atoms[globalIndex2].identity.compoundAtomIndex;
				const auto aIx3 = atoms[globalIndex3].identity.compoundAtomIndex;
