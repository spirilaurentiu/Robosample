...
auto EnergySnapshot::finite() const -> bool {
    ...
    if (!std::isfinite(potential)) {
        // std::cerr << "\t[WARNING] Potential energy is not finite: " << potential << '\n';
        ok = false;
    }
    if (!std::isfinite(kinetic)) {
        // std::cerr << "\t[WARNING] Kinetic energy is not finite: " << kinetic << '\n';
        ok = false;
    }
    if (!std::isfinite(fixman)) {
        // std::cerr << "\t[WARNING] Fixman energy is not finite: " << fixman << '\n';
        ok = false;
    }
    if (!std::isfinite(logSineSqrGamma2)) {
        // std::cerr << "\t[WARNING] logSineSqrGamma2 is not finite: " << logSineSqrGamma2 << '\n';
        ok = false;
    }
    if (!std::isfinite(total)) {
        // std::cerr << "\t[WARNING] Total energy is not finite: " << total << '\n';
        ok = false;
    }
    return ok;
}
...
auto EnergySnapshot::checkTotal(SimTK::Real RT) const -> bool {
    ...
    if (std::abs(total - expected) > tol) {
        std::cerr << "\t[WARNING] Total energy does not match sum of components: " << total << " vs "
                  << expected << '\n';
        return false;
    }
    return true;
...
auto EnergySnapshot::checkKinetic() const -> bool {
    ...
    if (kinetic < SimTK::Real{0}) {
        std::cerr << "\t[WARNING] Kinetic energy is negative: " << kinetic << '\n';
        return false;
    }
    return true;
}
...
auto EnergySnapshot::checkTermStability(const EnergySnapshot& ref) const -> bool {
    ...
    if (is_exploded(potential, ref.potential)) {
        // std::cerr << "\t[WARNING] Potential energy exploded: " << potential << " vs ref " << ref.potential
        //           << '\n';
        ok = false;
    }
    if (is_exploded(kinetic, ref.kinetic)) {
        // std::cerr << "\t[WARNING] Kinetic energy exploded: " << kinetic << " vs ref " << ref.kinetic <<
        // '\n';
        ok = false;
    }
    if (is_exploded(fixman, ref.fixman)) {
        ...
        // std::cerr << "\t[WARNING] Fixman energy exploded: " << fixman << " vs ref " << ref.fixman << '\n';
        ok = false;
    }
    if (is_exploded(total, ref.total)) {
        // std::cerr << "\t[WARNING] Total energy exploded: " << total << " vs ref " << ref.total << '\n';
        ok = false;
    }
...
auto EnergySnapshot::checkGeometryStability() const -> bool {
    ...
    if (std::abs(logSineSqrGamma2) > GEOMETRIC_LIMIT) {
        // std::cerr << "\t[WARNING] logSineSqrGamma2 too large: " << logSineSqrGamma2 << '\n';
        ok = false;
    }
    if (std::abs(fixman) > GEOMETRIC_LIMIT) {
        // std::cerr << "\t[WARNING] Fixman energy too large: " << fixman << '\n';
        ok = false;
    }
    return ok;
}
...
auto EnergySnapshot::hamiltonianDrift(const EnergySnapshot& ref,
    ...
    if (drift_kt > drift_limit) {
        std::cerr << "\t[WARNING] Hamiltonian drift too large: " << drift_kt << " kT (limit: " << drift_limit
                  << " kT)" << '\n';
        return false;
    }
    return true;
...
auto EnergySnapshot::validate(const EnergySnapshot& ref, SimTK::Real RT, DegreesOfFreedom ndofs) const
    ...
    if (potential / ref.potential > 10) {
        std::cerr << "\t[WARNING] Potential energy is suspiciously high: " << potential << " vs ref "
                  << ref.potential << '\n';
        valid = false; // ADVISORY: this is a heuristic plausibility check, not a hard rejection criterion
    }
    // On initialization, reference is 0
    if (ref.kinetic != 0) {
        if (kinetic / ref.kinetic > 10) {
            std::cerr << "\t[WARNING] Kinetic energy is suspiciously high: " << kinetic << " vs ref "
                      << ref.kinetic << '\n';
            valid = false; // ADVISORY: this is a heuristic plausibility check, not a hard rejection criterion
        }
    }
