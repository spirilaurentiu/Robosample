...
bool Context::initializeFromFile(const std::string &inpFN)
{
	...
	for (int worldIx = 0; worldIx < nofWorlds; worldIx++) {
		worlds[worldIx].addSampler(
			SamplerName::HMC,
			integratorTypes[setupReader.get("INTEGRATORS")[worldIx]],
		...
		|| true)
	{
		...
		for(int k = 0; k < temperatures.size(); k++){
			std::vector<AcceptRejectMode> modes;
			for(const auto& m : rexSamplers[k]){
				modes.push_back(acceptRejectModes[m]);
			}

			std::vector<IntegratorType> integrators;
			for(const auto& i : rexIntegrators[k]){
				integrators.push_back(integratorTypes[i]);
			}

			addThermodynamicState(k,
				temperatures[k],
				modes,
				rexDistortOptions[k],
				rexDistortArgs[k],
...
std::string Context::OMMRef_initialize(void)
{
		...
		for (auto atom : atoms){   
			...
			// 	<< aCnt <<" " << sqrtCoulombScale <<" "
			// 	<< charge <<" " << sigma <<" " 
			// 	<< vdwGlobalScaleFactor <<" " << wellDepth <<" "
			// 	<< std::endl;			
		}

		// TOPOLOGY: ADD BONDS (1-2)
		...
		// FORCES: ADD BONDS (1-2)
		for (auto bond : bonds) {
			SimTK::Real bondEquil = bond.getForceEquil() * DuMM::Ang2Nm;
			SimTK::Real bondK = bond.getForceK() * 2.0 * DuMM::Kcal2KJ/square(DuMM::Ang2Nm);

			// Ang and KcalPerAngstrom2
			ommHarmonicBondStretch->addBond(bond.i, bond.j,
									bondEquil , // * OpenMM::NmPerAngstrom,
									bondK ); // * OpenMM::KJPerKcal OR * OpenMM::AngstromsPerNm * OpenMM::AngstromsPerNm);
			
			// std::cout << "OMMRef_initialize addBond: " << bond.i <<" " << bond.j <<" "
			// 	<< bondEquil <<" " << bondK
			// 	<< std::endl;
		} // _end_ bonds
		...
		for (auto dummAngle : dummAngles) {
			...
			SimTK::Real theta0 = dummAngle.equil * DuMM::Deg2Rad;
			SimTK::Real forceKt = dummAngle.k * DuMM::Kcal2KJ * 2.0;
			...
			// std::cout << "OMMRef_initialize addAngle: " << a1num <<" " << a2num <<" "<< a3num
			// 	<<" "<< theta0 <<" "<< forceKt
			// 	<< std::endl;
		} // _end_ angles

		// FORCES: ADD DIHEDRALS (1-2-3-4)
		for (auto dummTorsion : dummTorsions) {
			...
			for(int pIx = 0; pIx < dummTorsion.period.size(); pIx++){

				int periodicity = dummTorsion.period[pIx];
...
void Context::prepareExchangePairs(int rexRound, int divisor, int oddity)
{
    exchangePairList.clear();
	...
	//int startIdx = (rexRound + oddity) % 2;
	int startIdx = oddity;
...
void Context::mixReplicas(int mixi, int oddity)
{
    ...
    if (getRunType() == RUN_TYPE::DEFAULT || nofReplicas <= 1) return;
...
void Context::setReplicasWorldsParameters(int thisReplica, bool alwaysAccept, bool adaptTimestep, int mixi)
{
	...
	for(std::size_t i = 0; i < replicaNofWorlds; i++){
		...
		if (worlds[replicaWorldIxs[i]].updSampler(0)->integratorType == IntegratorType::OMMVV) {
			worlds[replicaWorldIxs[i]].updSampler(0)->dumm->setOpenMMTimestep(
				timestep //  / 10
			);
		}
	}
...
RUN_TYPE Context::setRunType(const std::string& runTypeArgStr)
{
	...
	}else if(runTypeArgStr == "REBASONTOP"){
		this->runType = RUN_TYPE::REBASONTOP;
	}else{
		this->runType = RUN_TYPE::DEFAULT;
	}
...
/* bool CHECK_COORDS_FLAG = false;
#pragma CHECK_COORDS_0
if(CHECK_COORDS_FLAG){
	...
	std::cout << "CHECK_COORDS 0 "
		...
		<< std::endl;
	printStatus(checkCoords_wIx);
	std::cout << "CHECK_Context::RunREX: stage time " <<" "<< checkCoords_state.getSystemStage() <<" "<< checkCoords_state.getTime() << std::endl << std::flush;
	std::cout << "CHECK_Context::RunREX q u ValVersion " << checkCoords_state.getQValueVersion() <<" "<< checkCoords_state.getUValueVersion() << std::endl << std::flush;	
}
#pragma endregion CHECK_COORDS_0
*/
		...
		bool isNonEquilibrium)
{
			...
			for(int thWCnt = startWorldCnt; thWCnt < nofWorldsCounted; thWCnt++){
				...
				#pragma region ROLL_SET
				if(thermoIx != 0){
					if(worldsRolls[wIx] == true){
						SimTK::Real roll_ts = (currWorld.getSampler(0))->getTimestep();
						int roll_NU = currWorld.getNUs();
						currWorld.updIsRoll(false);
						(currWorld.updSampler(0))->setTimestep(roll_ts / roll_NU, false); ;
					}
				}
				#pragma endregion ROLL_SET

				bool validated = true;
				validated = RunWorld(wIx, string("REX, ") + to_string(replicaIx) + string(", ") + to_string(thermoIx) + ", " + to_string(wIx)) && validated;
				
				#pragma region ROLL_RESET
				if(thermoIx != 0){
					if(worldsRolls[wIx] == true){
						SimTK::Real roll_ts = (currWorld.getSampler(0))->getTimestep();
						int roll_NU = currWorld.getNUs();
						currWorld.updIsRoll(true);
						(currWorld.updSampler(0))->setTimestep(roll_ts * roll_NU, false);
					}
				}
				#pragma endregion ROLL_RESET
				...
				// ======================== NON-EQUILIBRIUM TRANSFER TO REPLICA ======================
					...
					transferCoordinates_WorldToReplica(wIx, replicaIx, intoWORK);
...
void Context::RunREX(int equilRounds, int prodRounds, int nofREXes)
{
	...
	// @@@@@@@@@@ LOOP THROUGH ROUNDS --------------------------------------------------------->
	int mixi = 0;
	for(size_t cyclIx = 0; cyclIx < equilRounds + prodRounds; cyclIx++) {

		// Update work scale factors
		//prepareExchangePairs(mixi, 2, 0); // divisor=2, oddity=0 swaps: (0,1), (2,3), (4,5)...
		...
		for (int replicaIx = 0; replicaIx < nofReplicas; replicaIx++){ // BY_REPLICA
			...
			// # pragma region HARDCODED_ROLL_PARAMS // @TODO DELETE
			...
			// 	// 	<< std::endl << std::flush;
			// } // @TODO DELETE
			// # pragma endregion HARDCODED_ROLL_PARAMS // @TODO DELETE
		...
		if(getRunType() == RUN_TYPE::REBASONTOP) {
			...
			// Update work scale factors
			prepareExchangePairs(mixi, 4, (0)); // divisor=4, oddity=0 swaps: (0,2), (1,3), (4,6)...
...
void Context::transferCoordinates_WorldToReplica(int srcWIx, int replicaIx, bool intoWORK)
{
	Replica& replica = replicas[replicaIx];	
	World& currWorld = worlds[srcWIx];
	HMCSampler* sampler_p = worlds[srcWIx].samplers[0].get();
	SimTK::State& advState = worlds[srcWIx].integ->updAdvancedState();

	if(intoWORK){
		replica.updWORK() += currWorld.getWork();  // TODO merge with Jacobians
		replica.upd_WORK_Jacobian() += sampler_p->getDistortJacobianDetLog();		
		replica.upd_WORK_AtomsLocationsInGround(worlds[srcWIx].getCurrentAtomsLocationsInGround());
		...
		replica.updAtomsLocationsInGround(worlds[srcWIx].getCurrentAtomsLocationsInGround());
