	...
	const std::vector<bSpecificAtom>& bAtomList)
{
	...
	// First root atom: lowest index terminal atom
	sortByIndex(terminalAtoms, false);
	...
	else if (firstAtom.getNumBonds() == 1) {
		roots.back().second = bAtomList[firstAtom.neighborsIndex[0]].getNumber();
	} else {
		std::vector<AmberAtom> candidates;
		for (auto i : firstAtom.neighborsIndex) {
			AmberAtom a(bAtomList[i]);
			if (!isTerminal(a)) {
				candidates.push_back(a);
			}
		}
		sortByIndex(candidates, false);
		roots.back().second = candidates[0].amberId;
	}
...
*/
bool InternalCoordinates::computeRoot_old(
	const std::vector<bSpecificAtom>& bAtomList)
{
	// Get terminal atoms
	std::vector<AmberAtom> terminalAtoms;
 ...
 * to calculate it into a level / offset format
 --> */
void InternalCoordinates::computeBAT_old(std::vector<bSpecificAtom>& bAtomList) {
...
/*!
 * <!-- Sort atoms in place first by mass and then byindeces --> */
...
*/
void InternalCoordinates::computeLevelsAndOffsets_old(const std::vector<bSpecificAtom>& bAtomList) {
	// adj list is bonds
 ...
 * <!-- Sort atoms in place first by mass and then by indeces -->
*/
void InternalCoordinates::sortByIndex(std::vector<AmberAtom>& v, bool reverse)
{
	// Sort atoms first by mass and then by indeces
	std::sort(v.begin(), v.end(),
		[reverse](const AmberAtom& lhs, const AmberAtom& rhs)
	{
		if (lhs.amberId == rhs.amberId) {
			if (reverse){
				return lhs.mass > rhs.mass;
			}else{
				return lhs.mass < rhs.mass;
