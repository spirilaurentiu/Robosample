...
GCHMCIntegrator::GCHMCIntegrator(PyObject *universe, std::string ligdir, std::string gaff_fn)
{
  ...
  bPYCHPRINT(pyo_universe_spec); //LS
  ...
  // LS
  std::cout<<"configuration->nd "<<configuration->nd<<std::endl<<std::flush;
  std::cout<<"configuration->dimensions[0] "<<(configuration->dimensions)[0]<<std::endl<<std::flush;
  std::cout<<"configuration->dimensions[1] "<<(configuration->dimensions)[1]<<std::endl<<std::flush;
  for(int i=0; i<(configuration->dimensions)[0]; i++){
    for(int j=0; j<(configuration->dimensions)[1]; j++){
      std::cout<<" data["<<(configuration->dimensions)[1]*i + j<<"] "<<std::flush;
      std::cout<<((double *)configuration->data)[(configuration->dimensions)[1]*i + j]<<std::flush;
    }
    std::cout<<endl;
  }
  // LS
