	...
	if (connectivity.numBondsInvolved > 0) {
		if (connectivity.numBondsInvolved == 1) {
			...
			compoundSingleAtom->addFirstTwoBondCenters(
				"bond1", "bond2", identity.uniqueAtomName,
			...
			if (connectivity.numBondsInvolved > 2) {
				...
				compoundSingleAtom->addLeftHandedBondCenter( "bond3", identity.uniqueAtomName, TetrahedralAngle, TetrahedralAngle
				);
			}
			...
			if (connectivity.numBondsInvolved > 3) {
				...
				compoundSingleAtom->addRightHandedBondCenter("bond4", identity.uniqueAtomName, TetrahedralAngle, TetrahedralAngle
				);
			}
		}
