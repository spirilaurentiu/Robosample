...
Sampler::Sampler(World& argWorld,
    ...
    int ThreeFrom3D = 3;
    numDegreesOfFreedom = natoms * ThreeFrom3D;
...
bool Sampler::getAlwaysAccept() const {
    return alwaysAccept;
}
...
// void Sampler::update(SimTK::State& someState){}
...
SimTK::Quaternion Sampler::generateRandomQuaternion() {
...
// Just for checking
void Sampler::checkAtomStationsThroughDumm() {
...
std::vector<double>& Sampler::generateVonMisesFisherSample(std::vector<double>& X, double lambda) {
    ...
    double ndofs_1 = numDegreesOfFreedom - 1;
    ...
    for (int t = 0; t < nofTries; t++) {
        ...
        if (((lambda * W) + LOG - c) >= std::log(U)) {
            ...
            // Actual sample
            multiplyByScalar(V, std::sqrt(1 - (W * W)), X_lowdim);
            for (int j = 1; j < numDegreesOfFreedom; j++) {
                X[j] = X_lowdim[j - 1];
            }
            X[0] = W;
...
// Draws from von Mises-Fisher distribution
double Sampler::generateChiSample() {
    // TODO
...
SimTK::Real Sampler::convoluteVariable(SimTK::Real& passedVariable,
    ...
    // Gamma distribution
        ...
        std::cerr << "Sampler distribute variable: Unknown distribution\n";
    }

    return passedVariable;
}

SimTK::Real Sampler::convoluteVariable(std::vector<SimTK::Real>& vvar,
                                       std::string distrib,
                                       SimTK::Real param1,
                                       SimTK::Real param2) {
    for (auto& var : vvar) {
...
SimTK::Real Sampler::calcDeformationPotential(SimTK::Real& var,
                                              std::string distrib,
                                              SimTK::Real param1,
                                              SimTK::Real param2) {
