// ============================================================================
//  TestRoboticsOracleMoleculeNumeric.cpp -- Scope B (molecule) of the
//  robotics oracle (docs/specs/robotics-oracle-differential.md §4.2/§4.4/
//  §5.1), NUMERIC-differential family, split out of
//  TestRoboticsOracleMolecule.cpp (TEST-005).
//
//  Performs the live-Simbody NUMERIC differential (§6 staged comparison,
//  "Scope B" proper): tests/fixtures/robotics_oracle_molecules/
...
//  The structural Free-root check (numBodies/nu/nq) is covered, port-only,
//  by TestRoboticsOracleMoleculeSymbolic.cpp's RigidFreeRoot; this file's
//  RigidWeldRootNumeric covers the Weld-root rigid class's numeric
//  differential (0 DOF, trivial but real: it pins X_GB / mass-property
//  agreement for the single-rigid-body decomposition).
//
...
//
//  *** FINDING #1, CORRECTED (coder investigation -- see the coder
//  checkpoint for the full residual table). SUPERSEDES a previous
//  "CONFIRMED FINDING #1" that misattributed the cause to the port's
//  `g_self` frame construction; that attribution is REFUTED by direct
...
//  44 g_self bodies with no cancellation (same mechanism the Scope-A oracle's
//  `kLogDetMFuzzTol`/FuzzTopo06 already documents), reaching residuals up to
//  ~3.1e-4 (A_GB, abs) / ~2.4e-4 (udot, abs) / ~6.8e-4 (logDetM, abs, ~5.6e-5
//  relative) / ~3.6e-2 (KE, abs, ~4.1e-5 relative) at the random state -- NOT
//  present in REST's velocity terms (u=0 there) but logDetM (q-only, no
//  velocity) already carries the SAME ~1.7e-5-relative residual at REST,
//  confirming the mechanism is q-space (oracle mass-property
//  reconstruction), not "velocity-coupled amplification" as previously
//  stated. This is a property of the ORACLE reference for this molecule, not
//  the port, so it is handled by a documented, CASE-SPECIFIC looser
//  tolerance (`kMolFlexibleStage4Tol`/`kMolFlexibleLogDetMRelTol` below)
//  passed only to this one test -- the shared `kMolStage4Tol`/
...
//
//  STATUS: RegularFlexibleNumeric is ENABLED using the case-specific
//  tolerances above.
//
//  *** ADJUDICATED VERDICT on the former "FINDING #2" (hostile-review
//  verdict, refined spec §6): Finding #1 above was subsequently CORRECTED,
//  see "FINDING #1, CORRECTED" ***
...
//  of the (tree-wide, aggregate) singular-tree guard (cause (a) above), exactly
//  3 of 1APQ's 335 bodies (0.9%) fail the `V_GB.angular` comparison at the
//  random state, each by EXACTLY 2*|u_body| (verified: 0.6067 vs
//  2*0.30335=0.6067; 1.1774 vs 2*0.58868=1.1774; 0.9932 vs 2*0.49659=0.9932).
//  All 3 are single-atom, childless Torsion bodies whose ONLY non-tree bond
//  is the ring-closing bond itself (atoms 377, 331, 699 -- endpoints of
//  ring-closing pairs (377,378)/(331,332)/(699,700)) -- clone port/clone
//  MobilizedBodyIndex 68/72, 93/93, 280/281. Per docs/specs/singular-dof-
//  fixman.md: this is EXACTLY the structural-phantom-DOF geometry (C1.i, "a
//  leaf single-atom Torsion body whose atom lies on the rotation axis") --
//  the atom sits ON its own hinge axis, so the torsion moves it nowhere
//  (confirmed: per-atom Ground positions for all 3 agree at Stage 1, to the
//  SAME tolerance as every other body) and the hinge inertia `D_b` is null
//  (per-body `minEigD` ~1.019e-33 for all 3, 21+ orders of magnitude below
//  the tolerance below). `V_GB.angular = axis*u` is therefore a GAUGE
//  quantity for these 3 bodies specifically: the hinge axis DIRECTION is
//  physically unobservable (the mobilizer moves no mass either way), and the
//  port's and clone's independent `f_self`/`r_self`-vs-BondCenter fallback-
//  bucket axis derivations (same root-cause LOCATION, `recomputeGeometry`'s
//  childless-body fallback) are free to disagree on it -- the EXACT 2*|u|
//  signature is a sign flip of an otherwise-correct axis LINE, consistent
//  with the "ADJUDICATED VERDICT" note above (0/334 axis-LINE mismatches).
//  Handled PER-BODY (not tree-wide) in `runMoleculeNumericDifferential`: a
//  body whose own `minEigD <= kMolMinEigDEps` has its `V_GB.angular`/
//  `A_GB.angular` comparisons SKIPPED (loudly, Rule 11), while its
//  `V_GB.linear`/`A_GB.linear` (physical: a childless body's origin only
//  translates with its PARENT) and every OTHER (non-phantom) body's full
//  V_GB/A_GB stay compared at the UNCHANGED tolerances -- so Cyclic1APQNumeric
//  is no longer red on this account, and the guard remains discriminating: it
//  cannot mask a genuine divergence on any of the other 332 (non-phantom)
//  bodies.
// ============================================================================
#include <algorithm>
#include <cmath>
