...
TEST(Simbody_LinearBushing_ParameterSetting_DefaultAndStateConsistency,
     DefaultsPropagateAndStateOverridesWorkCorrectly) {
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);

    // This is the frame on body1.
    const Transform X_B1F(SimTK::Test::randRotation(), SimTK::Test::randVec3());

    // This is the frame on body 2.
    const Transform X_B2M(SimTK::Test::randRotation(), SimTK::Test::randVec3());
    ...
    Body::Rigid aBody(MassProperties(Mass, Vec3(.1, .2, .3), Mass * Inertia(1, 1.1, 1.2, 0.01, 0.02, 0.03)));

    MobilizedBody::Free body1(matter.Ground(), Transform(), aBody, Transform());
    MobilizedBody::Free body2(matter.Ground(), Transform(), aBody, Transform());

    Force::LinearBushing bushing(forces, body1, Vec3(1, 2, 3), body2, Vec3(-2, -3, -4), Vec6(5), Vec6(7));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDefaultFrameOnBody1(), Transform(Vec3(1, 2, 3)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDefaultFrameOnBody2(), Transform(Vec3(-2, -3, -4)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDefaultStiffness(), Vec6(5, 5, 5, 5, 5, 5));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDefaultDamping(), Vec6(7, 7, 7, 7, 7, 7));
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDefaultFrameOnBody1(), Transform(Vec3(-1, -2, -3)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDefaultFrameOnBody2(), Transform(Vec3(2, 3, 4)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDefaultStiffness(), Vec6(9));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDefaultDamping(), Vec6(11));
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getFrameOnBody1(state), Transform(Vec3(-1, -2, -3)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getFrameOnBody2(state), Transform(Vec3(2, 3, 4)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getStiffness(state), Vec6(9));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDamping(state), Vec6(11));
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getFrameOnBody1(state), X_B1F);
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getFrameOnBody2(state), X_B2M);
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getStiffness(state), k);
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDamping(state), c);
}
...
TEST(Simbody_LinearBushing_KinematicsAndEnergy, MatchesEquivalentMobilizedChainAndConservesEnergyOverTime) {
    // Build system.
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);
    ...
    Body::Rigid aBody(MassProperties(Mass, Vec3(.1, .2, .3), Mass * Inertia(1, 1.1, 1.2, 0.01, 0.02, 0.03)));
    aBody.addDecoration(Transform(), DecorativeEllipsoid(HalfShape).setOpacity(0.25).setColor(Blue));
    aBody.addDecoration(X_B1F, DecorativeFrame(0.5).setColor(Red));
    aBody.addDecoration(X_B2M, DecorativeFrame(0.5).setColor(Green));

    // Ground attachment frame.
    const Transform X_GA(SimTK::Test::randRotation(), SimTK::Test::randVec3());

    MobilizedBody::Free body1(matter.Ground(), X_GA, aBody, X_B2M);

    // This is to keep the system from flying away.
    Force::TwoPointLinearSpring(forces, matter.Ground(), Vec3(0), body1, Vec3(0), 10, 0);

    Body::Rigid massless(MassProperties(0, Vec3(0), Inertia(0)));
    const Rotation ZtoX(Pi / 2, YAxis);
    const Rotation ZtoY(-Pi / 2, XAxis);
    MobilizedBody::Cartesian dummy1(body1, X_B1F, massless, Transform());
    MobilizedBody::Pin dummy2(dummy1, ZtoX, massless, ZtoX);
    MobilizedBody::Pin dummy3(dummy2, ZtoY, massless, ZtoY);
    MobilizedBody::Pin body2(dummy3,
                             Transform(), // about Z
                             aBody,
                             X_B2M);

    // Set the actual parameters in the State below.
    Force::LinearBushing bushing(forces, body1, body2, Vec6(0), Vec6(0));
    ...
    Vector xyz = istate.getQ()(7, 3);
    Vector ang = istate.getQ()(10, 3);
    Vector xyzd = istate.getQDot()(7, 3);
    Vector angd = istate.getQDot()(10, 3);
    Vec6 mq = Vec6(ang[0], ang[1], ang[2], xyz[0], xyz[1], xyz[2]);
    Vec6 mqd = Vec6(angd[0], angd[1], angd[2], xyzd[0], xyzd[1], xyzd[2]);

    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getQ(istate), mq);
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getQDot(istate), mqd);

    const Real initialEnergy = system.calcEnergy(istate);

    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getDissipatedEnergy(istate), 0);
    ...
    system.realize(istate, Stage::Velocity);
    xyz = istate.getQ()(7, 3);
    ang = istate.getQ()(10, 3);
    xyzd = istate.getQDot()(7, 3);
    angd = istate.getQDot()(10, 3);
    ...
    const Real finalEnergy = system.calcEnergy(istate) + bushing.getDissipatedEnergy(istate);

    EXPECT_NEAR_CUSTOM_TOL_SIMTK(initialEnergy, finalEnergy, 10 * Accuracy);
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(X_GF, bushing.getX_GF(istate));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(X_GM, bushing.getX_GM(istate));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(~X_GF * X_GM, bushing.getX_FM(istate));
    ...
    const SpatialVec V_GF(V_GB1[0], body1.findStationVelocityInGround(istate, X_B1F.p()));
    const SpatialVec V_GM(V_GB2[0], body2.findStationVelocityInGround(istate, X_B2M.p()));

    EXPECT_NEAR_DEFAULT_TOL_SIMTK(V_GF, bushing.getV_GF(istate));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(V_GM, bushing.getV_GM(istate));

    const SpatialVec V_FM(~X_B1F.R() * body2.findBodyAngularVelocityInAnotherBody(istate, body1),
                          ~X_B1F.R() * body2.findStationVelocityInAnotherBody(istate, X_B2M.p(), body1));

    EXPECT_NEAR_DEFAULT_TOL_SIMTK(V_FM, bushing.getV_FM(istate));
}
...
// for a while and see that energy+dissipation is conserved.
TEST(Simbody_LinearBushingMobilizer_EquivalenceAndEnergyConservation,
     MatchesBushingMobilizerKinematicsAndConservesEnergy) {
    // Build system.
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);
    ...
    MobilizedBody::Bushing body2(body1, X_B1F, aBody, X_B2M);
...
// LinearBushing force.
TEST(Simbody_LinearBushing_ReverseConfiguration, MatchesEquivalentMobilitySpringsAndDampersForces) {
    // Create the system.
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);
    ...
    Body::Rigid brickBody(
        MassProperties(Mass, Vec3(.1, .2, .3), Mass * Inertia(1, 1.1, 1.2, 0.01, 0.02, 0.03)));
    // Body::Rigid brickBody(MassProperties(Mass, Vec3(0),
    //                         Mass*UnitInertia::ellipsoid(HalfShape)));
    brickBody.addDecoration(Transform(), DecorativeEllipsoid(HalfShape).setOpacity(0.25).setColor(Blue));
    brickBody.addDecoration(BodyAttach, DecorativeFrame(0.5).setColor(Red));

    MobilizedBody::Free brick1(matter.Ground(), Transform(), brickBody, BodyAttach);
    ...
    MobilizedBody::Pin dummy1(matter.Ground(),
                              Vec3(1, 1, 1) + Vec3(2, 0, 0),
                              massless,
                              Transform(),
                              MobilizedBody::Reverse);
    MobilizedBody::Pin dummy2(dummy1, ZtoY, massless, ZtoY, MobilizedBody::Reverse);
    MobilizedBody::Pin dummy3(dummy2, ZtoX, massless, ZtoX, MobilizedBody::Reverse);
    MobilizedBody::Cartesian brick2(dummy3, Transform(), brickBody, BodyAttach, MobilizedBody::Reverse);

    const Vec6 k = SimTK::Test::randVec<6>().abs(); // must be > 0
    const Vec6 c = SimTK::Test::randVec<6>().abs(); // "
    Transform GroundAttach(Rotation(), Vec3(1, 1, 1));
    matter.Ground().updBody().addDecoration(GroundAttach, DecorativeFrame(0.5).setColor(Green));
    ...
    Force::LinearBushing bushing(forces, brick1, BodyAttach, matter.Ground(), GroundAttach, k1, c1);

    Force::LinearBushing bushing2(forces, brick1, brick2, k, c);

    Transform GroundAttach2(Rotation(), Vec3(1, 1, 1) + Vec3(2, 0, 0));
    matter.Ground().updBody().addDecoration(GroundAttach2, DecorativeFrame(0.5).setColor(Green));
    const Vec6 k2 = k;
    const Vec6 c2 = c;
    ...
    const Rotation RR = SimTK::Test::randRotation();
    brick1.setQToFitTransform(state, RR);

    const Vec3 qRRinv = Rotation(~RR).convertRotationToBodyFixedXYZ();
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(bushing.getF_GF(state), reactions[brick2.getMobilizedBodyIndex()]);
}
...
// make sure the mobilizer reaction forces match the corresponding
// LinearBushing force.
TEST(Simbody_LinearBushing_ReverseMobilizer, MatchesMobilizerReactionForces) {
    // Create the system.
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);
    ...
    matter.Ground().addBodyDecoration(Transform(), DecorativeFrame(.25).setColor(Purple));
    ...
    MobilizedBody::Bushing brick2(matter.Ground(),
                                  Vec3(1, 1, 1) + Vec3(2, 0, 0),
                                  brickBody,
                                  BodyAttach,
                                  MobilizedBody::Reverse);
    ...
    const Vec6 k = SimTK::Test::randVec<6>().abs();
    const Vec6 c = SimTK::Test::randVec<6>().abs();
    Transform GroundAttach(Rotation(), Vec3(1, 1, 1));
    matter.Ground().updBody().addDecoration(GroundAttach, DecorativeFrame(0.5).setColor(Green));
    ...
    // the F frame is moving.
    Force::LinearBushing bushing(forces,
                                 brick1,
                                 BodyAttach, // .5,0,0
                                 matter.Ground(),
                                 GroundAttach, // 1,1,1
