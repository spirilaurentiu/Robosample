...
void compareReactionToConstraint(SpatialVec reactionForce, const Constraint& constraint, const State& state) {
    ...
    constraint.calcConstraintForcesFromMultipliers(state,
                                                   constraint.getMultipliersAsVector(state),
                                                   constraintForce,
                                                   mobilityForce);
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(reactionForce,
                                  -1 * constraint.getAncestorMobilizedBody().getBodyRotation(state)
                                      * constraintForce[1]);
}
 ...
 */
TEST(Simbody_MobilizerReactionForces_EquivalentMobilizersAndConstraints,
     ReactionForcesMatchConstraintsAndFreeBodiesHaveZero) {
    ...
    MobilizedBody::Free fb2(fb1,
                            Transform(Vec3(0, 0, BOND_LENGTH)),
                            body,
    ...
    Constraint::ConstantOrientation ft1constraint(matter.updGround(),
                                                  Rotation(0, Vec3(1)),
                                                  ft1,
                                                  Rotation(0, Vec3(1)));
    Constraint::ConstantOrientation ft2constraint(ft1, Rotation(0, Vec3(1)), ft2, Rotation(0, Vec3(1)));
    MobilizedBody::Translation t1(matter.updGround(),
                                  Transform(Vec3(0)),
                                  body,
                                  Transform(Vec3(BOND_LENGTH, 0, 0)));
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(b1.getBodyTransform(state), fb1.getBodyTransform(state));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(b2.getBodyTransform(state), fb2.getBodyTransform(state));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(b1.getBodyVelocity(state), fb1.getBodyVelocity(state));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(b2.getBodyVelocity(state), fb2.getBodyVelocity(state));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(t1.getBodyTransform(state), ft1.getBodyTransform(state));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(t2.getBodyTransform(state), ft2.getBodyTransform(state));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(t1.getBodyVelocity(state), ft1.getBodyVelocity(state));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(t2.getBodyVelocity(state), ft2.getBodyVelocity(state));
    ...
    for (MobilizedBodyIndex bx(0); bx < matter.getNumBodies(); ++bx) {
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(
            forcesAtMInG[bx],
            matter.getMobilizedBody(bx).findMobilizerReactionOnBodyAtMInGround(state));
    }

    // Make sure all free bodies have no reaction force on them.
    EXPECT_NEAR_DEFAULT_TOL_SIMTK((forcesAtMInG[f1.getMobilizedBodyIndex()]), SpatialVec(Vec3(0), Vec3(0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK((forcesAtMInG[f2.getMobilizedBodyIndex()]), SpatialVec(Vec3(0), Vec3(0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK((forcesAtMInG[fb1.getMobilizedBodyIndex()]), SpatialVec(Vec3(0), Vec3(0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK((forcesAtMInG[fb2.getMobilizedBodyIndex()]), SpatialVec(Vec3(0), Vec3(0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK((forcesAtMInG[ft1.getMobilizedBodyIndex()]), SpatialVec(Vec3(0), Vec3(0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK((forcesAtMInG[ft2.getMobilizedBodyIndex()]), SpatialVec(Vec3(0), Vec3(0)));
 ...
 */
TEST(Simbody_MobilizerReactionForces_PinVsConstrainedFreePendulum,
     ReactionForcesMatchAndFrameShiftsAreConsistent) {
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);
    ...
    MobilizedBody::Free pendulum2(matter.updGround(),
                                  ...
                                  pendulumBody,
                                  Transform(Vec3(0, 1, 0)));
    Constraint::Ball ballcons2(matter.updGround(), Vec3(2, -1, 0), pendulum2, Vec3(0, 1, 0));
    const Transform& X_GF2 = pendulum2.getDefaultInboardFrame();
    const Transform& X_P2M = pendulum2.getDefaultOutboardFrame();
    Constraint::ConstantAngle angx2(matter.Ground(), X_GF2.x(), pendulum2, X_P2M.z());
    Constraint::ConstantAngle angy2(matter.Ground(), X_GF2.y(), pendulum2, X_P2M.z());

    MobilizedBody::Free pendulum2b(pendulum2,
                                   Transform(x45, Vec3(0, -1, 0)),
                                   pendulumBody,
                                   Transform(Vec3(0, 1, 0)));
    Constraint::Ball ballcons2b(pendulum2, Vec3(0, -1, 0), pendulum2b, Vec3(0, 1, 0));
    const Transform& X_GF2b = pendulum2b.getDefaultInboardFrame();
    const Transform& X_P2Mb = pendulum2b.getDefaultOutboardFrame();
    Constraint::ConstantAngle angx2b(pendulum2, X_GF2b.x(), pendulum2b, X_P2Mb.z());
    Constraint::ConstantAngle angy2b(pendulum2, X_GF2b.y(), pendulum2b, X_P2Mb.z());
    ...
    Vector_<SpatialVec> forcesAtBInG;
    Vector_<SpatialVec> forcesAtFInG;
    matter.calcMobilizerReactionForces(state, forcesAtMInG);
    ...
    for (MobilizedBodyIndex mbx(0); mbx < matter.getNumBodies(); ++mbx) {
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(
            forcesAtMInG[mbx],
            matter.getMobilizedBody(mbx).findMobilizerReactionOnBodyAtMInGround(state));

        forcesAtFInG[mbx] = matter.getMobilizedBody(mbx).findMobilizerReactionOnParentAtFInGround(state);
    }
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(forcesAtFInG, forcesAtFInG_byhand);
    ...
    forcesAtBInG[p1x] = shiftForceFromTo(forcesAtMInG[p1x], R_G1 * Vec3(0, 1, 0), Vec3(0));
    forcesAtBInG[p1bx] = shiftForceFromTo(forcesAtMInG[p1bx], R_G1b * Vec3(0, 1, 0), Vec3(0));

    // Compare those manually-shifted quantities to the ones we can get
    // direction from the MobilizedBody.
    SpatialVec forcesAtBInG_p1 = pendulum1.findMobilizerReactionOnBodyAtOriginInGround(state);
    SpatialVec forcesAtBInG_p1b = pendulum1b.findMobilizerReactionOnBodyAtOriginInGround(state);

    EXPECT_NEAR_DEFAULT_TOL_SIMTK(forcesAtBInG[p1x], forcesAtBInG_p1);
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(forcesAtBInG[p1bx], forcesAtBInG_p1b);
    ...
    Vector_<SpatialVec> cons2Forces =
        -(ballcons2.getConstrainedBodyForcesAsVector(state) + angx2.getConstrainedBodyForcesAsVector(state)
          + angy2.getConstrainedBodyForcesAsVector(state));
    Vector_<SpatialVec> cons2bForces =
        -(ballcons2b.getConstrainedBodyForcesAsVector(state) + angx2b.getConstrainedBodyForcesAsVector(state)
          + angy2b.getConstrainedBodyForcesAsVector(state));

    // Couldn't quite make default tolerance on some platforms. This uses 10X default.
    EXPECT_SIMTK_SIZE(cons2Forces[1], forcesAtBInG[p1x], 10);
    EXPECT_SIMTK_SIZE(cons2bForces[1], forcesAtBInG[p1bx], 10);
}

/**
 * Construct a system of several bodies, and compare the reaction forces to those calculated by SD/FAST.
 */
TEST(Simbody_MobilizerReactionForces_MatchesSDFASTReference,
     ReactionForcesMatchAcrossConfigurationsAndVelocities) {
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);
    Force::UniformGravity(forces, matter, Vec3(0, -9.8, 0));
    ...
    Inertia inertia = Inertia(Mat33(0.1, 0.01, 0.01, 0.01, 0.1, 0.01, 0.01, 0.01, 0.1));
    MobilizedBody::Slider body1(matter.updGround(), MassProperties(10.0, Vec3(0), inertia));
    MobilizedBody::Pin body2(body1,
                             Vec3(0.1, 0.1, 0),
                             MassProperties(20.0, Vec3(0), inertia),
                             Vec3(0, -0.2, 0));
    MobilizedBody::Gimbal body3(body2,
                                Vec3(0, 0.2, 0),
                                MassProperties(20.0, Vec3(0), inertia),
                                Vec3(0, -0.2, 0));
    MobilizedBody::Pin body4(body3,
                             Vec3(0, 0.2, 0),
                             MassProperties(30.0, Vec3(0), inertia),
                             Vec3(0, -0.2, 0));
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(~body1.getBodyTransform(state).R()
                                      * reaction[body1.getMobilizedBodyIndex()],
                                  SpatialVec(Vec3(0, 0, 68.6), Vec3(0, 784.0, 0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(~body2.getBodyTransform(state).R()
                                      * reaction[body2.getMobilizedBodyIndex()],
                                  SpatialVec(Vec3(0, 0, 0), Vec3(0, 686.0, 0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(~body3.getBodyTransform(state).R()
                                      * reaction[body3.getMobilizedBodyIndex()],
                                  SpatialVec(Vec3(0, 0, 0), Vec3(0, 490.0, 0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(~body4.getBodyTransform(state).R()
                                      * reaction[body4.getMobilizedBodyIndex()],
                                  SpatialVec(Vec3(0, 0, 0), Vec3(0, 294.0, 0)));
    ...
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(
        ~body1.getBodyTransform(state).R() * reaction[body1.getMobilizedBodyIndex()],
        SpatialVec(Vec3(1.647327, 0.783211, 34.088183), Vec3(0, 359.274099, 3.342380)),
        1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(
        ~body2.getBodyTransform(state).R() * reaction[body2.getMobilizedBodyIndex()],
        SpatialVec(Vec3(1.688077, 0.351125, 0), Vec3(55.399123, 267.455570, 3.342380)),
        1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body3.getBodyTransform(state).R() * reaction[body3.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(-17.757553, 174.663042, -11.383057)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(
        ~body4.getBodyTransform(state).R() * reaction[body4.getMobilizedBodyIndex()],
        SpatialVec(Vec3(0.910890, 0.082353, 0), Vec3(-13.977214, 74.444715, 4.943682)),
        1e-5);
    ...
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body1.getBodyTransform(state).R() * reaction[body1.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(-13.549253, 2.723897, -6.355912), Vec3(0, 34.0, -27.088584)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(
        ~body2.getBodyTransform(state).R() * reaction[body2.getMobilizedBodyIndex()],
        SpatialVec(Vec3(-10.840395, 0.015039, 0), Vec3(-0.440882, -64.0, -27.088584)),
        1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body3.getBodyTransform(state).R() * reaction[body3.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(0.692814, -256.000000, -27.088584)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body4.getBodyTransform(state).R() * reaction[body4.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(3.276930, -0.281928, 0), Vec3(3.796164, -372.0, 21.472977)),
                                 1e-5);
}
 ...
 * Construct a system of several bodies, and compare the reaction forces to those calculated by SD/FAST.
 */
TEST(Simbody_MobilizerReactionForces_FemurTibiaModelMatchesSDFAST,
     ReactionForcesMatchReferenceAcrossConfigurations) {
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);
    ...
    MobilizedBody::Pin p1(matter.Ground(),
                          Transform(Vec3(0.0000, -0.0700, 0.0935)),
                          femur,
                          Transform(Vec3(0.0020, 0.1715, 0)));
    MobilizedBody::Slider p2(p1,
                             Transform(Vec3(0.0033, -0.2294, 0)),
                             tibia,
                             Transform(Vec3(0.0, 0.1862, 0.0)));
    State state = system.realizeTopology();
    system.realize(state, Stage::Acceleration);
    ...
    Vector_<SpatialVec> reaction(matter.getNumBodies());
    matter.calcMobilizerReactionForces(state, reaction);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~p1.getBodyTransform(state).R() * reaction[p1.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(0.438079, 120.773069, 0)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~p2.getBodyTransform(state).R() * reaction[p2.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0.014040), Vec3(0, 34.422139, 0)),
                                 1e-5);
    ...
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~p1.getBodyTransform(state).R() * reaction[p1.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(-39.481457, 10.489344, 0)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~p2.getBodyTransform(state).R() * reaction[p2.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 1.502242), Vec3(0, 11.035987, 0)),
                                 1e-5);
}

/**
 * Construct a system of several bodies and a constraint, and compare the reaction forces to those calculated
 * by SD/FAST.
 */
TEST(Simbody_MobilizerReactionForces_WithRodConstraintMatchesSDFAST,
     ReactionForcesMatchReferenceAcrossConfigurations) {
    MultibodySystem system;
    SimbodyMatterSubsystem matter(system);
    GeneralForceSubsystem forces(system);
    ...
    MobilizedBody::Gimbal body2(body1,
                                Vec3(0, -0.1, 0.2),
                                MassProperties(20.0, Vec3(0), inertia),
                                Vec3(0, 0.2, 0));
    MobilizedBody::Gimbal body3(body1,
                                Vec3(0, -0.1, -0.2),
                                MassProperties(20.0, Vec3(0), inertia),
                                Vec3(0, 0.2, 0));
    MobilizedBody::Gimbal body4(body2,
                                Vec3(0, -0.2, 0),
                                MassProperties(30.0, Vec3(0), inertia),
                                Vec3(0, 0.2, 0));
    MobilizedBody::Gimbal body5(body3,
                                Vec3(0, -0.2, 0),
                                MassProperties(30.0, Vec3(0), inertia),
                                Vec3(0, 0.2, 0));
    ...
    matter.calcMobilizerReactionForces(state, reaction);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body1.getBodyTransform(state).R() * reaction[body1.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(-0.000626, 1077.988912, 0.000030)),
                                 1e-2);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body2.getBodyTransform(state).R() * reaction[body2.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(-0.005038, 495.288692, -18.767467)),
                                 1e-2);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body3.getBodyTransform(state).R() * reaction[body3.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(0.004236, 495.287857, 18.767535)),
                                 1e-2);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body4.getBodyTransform(state).R() * reaction[body4.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(0.006251, 303.365940, -0.202330)),
                                 1e-2);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body5.getBodyTransform(state).R() * reaction[body5.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(-0.005933, 303.365472, 0.202301)),
                                 1e-2);
    ...
    r.setRotationFromThreeAnglesThreeAxes(BodyRotationSequence,
                                          0.433843,
                                          ZAxis,
                                          0.647441,
                                          XAxis,
    ...
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body1.getBodyTransform(state).R() * reaction[body1.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(99.121319, 139.500095, 95.065409)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body2.getBodyTransform(state).R() * reaction[body2.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(15.359115, 55.876994, 22.508078)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body3.getBodyTransform(state).R() * reaction[body3.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(15.696393, 65.002065, 13.133021)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body4.getBodyTransform(state).R() * reaction[body4.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(-6.262023, 32.714510, -9.770708)),
                                 1e-5);
    EXPECT_NEAR_CUSTOM_TOL_SIMTK(~body5.getBodyTransform(state).R() * reaction[body5.getMobilizedBodyIndex()],
                                 SpatialVec(Vec3(0, 0, 0), Vec3(10.471620, 0.963822, -4.640161)),
                                 1e-5);
}

/*
 * Create a free body in space and apply some forces to it. As long as we don't apply mobility forces, the
 * reaction force in the mobilizer should be zero. It is important to do this with a full inertia, offset com,
 * non-unit mass, twisted frames, non-zero velocities, etc.
 */
TEST(Simbody_MobilizerReactionForces_FreeMobilizer, ReactionForcesAreZeroUnderAppliedForces) {
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(fwdReac[0], SpatialVec(Vec3(0)));
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(fwdReac[1], SpatialVec(Vec3(0)));
}
