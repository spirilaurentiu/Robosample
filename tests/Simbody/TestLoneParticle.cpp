...
static void compareToTranslate(bool prescribe, Motion::Level level) {
    ...
    EXPECT_NEAR_DEFAULT_TOL_SIMTK(reactionForces, reactionForcesFreebody);
    ...
    for (int i = 0; i < numBodies; i++) {
        ...
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(body1.getBodyOriginLocation(state), body2.getBodyOriginLocation(state));
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(body1.getBodyOriginVelocity(state), body2.getBodyOriginVelocity(state));
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(body1.getBodyOriginAcceleration(state),
                                      body2.getBodyOriginAcceleration(state));
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(reactionForces[index1][0], reactionForces[index2][0]);
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(reactionForces[index1][1], reactionForces[index2][1]);
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(Vec3::getAs(&mv[6 * i]), Vec3::getAs(&mv[6 * i + 3]));
        if (!prescribe) {
            EXPECT_NEAR_DEFAULT_TOL_SIMTK(Vec3::getAs(&minvv[6 * i]), Vec3::getAs(&minvv[6 * i + 3]));
        }
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(Vec3::getAs(&residualMobilityForces[6 * i]),
                                      Vec3::getAs(&residualMobilityForces[6 * i + 3]));
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(Vec3::getAs(&dEdQ[6 * i]), Vec3::getAs(&dEdQ[6 * i + 3]));
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(compositeInertias[index1].getMass(),
                                      compositeInertias[index2].getMass());
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(compositeInertias[index1].getMassCenter(),
                                      compositeInertias[index2].getMassCenter());
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(compositeInertias[index1].getUnitInertia().getMoments(),
                                      compositeInertias[index2].getUnitInertia().getMoments());
        EXPECT_NEAR_DEFAULT_TOL_SIMTK(compositeInertias[index1].getUnitInertia().getProducts(),
                                      compositeInertias[index2].getUnitInertia().getProducts());
    }
}

TEST(Simbody_LoneParticle_Free, MatchesTranslateModelAcrossAllQuantities) {
    compareToTranslate(false, Motion::Position);
}

TEST(Simbody_LoneParticle_PrescribedPosition, ConsistentWithTranslateKinematics) {
    compareToTranslate(true, Motion::Position);
}

TEST(Simbody_LoneParticle_PrescribedVelocity, ConsistentWithTranslateDynamicsAndKinematics) {
    compareToTranslate(true, Motion::Velocity);
}

TEST(Simbody_LoneParticle_PrescribedAcceleration, ConsistentUnderAccelerationLevelPrescriptions) {
