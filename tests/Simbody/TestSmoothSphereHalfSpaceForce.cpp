...
TEST(Simbody_SmoothSpehereHalfSpaceForce, testForces) {
    ...
    MobilizedBody::Translation sphere(matter.updGround(), Transform(), body1, Transform());

    Body::Rigid body2(MassProperties(1.0, Vec3(0), Inertia(1)));
    MobilizedBody::Free halfSpace(matter.updGround(), Transform(), body2, Transform());
    ...
    for (Real height = radius + 0.2; height > 0; height -= 0.1) {
        ...
        const auto v = system.getRigidBodyForces(state, Stage::Dynamics)[sphere.getMobilizedBodyIndex()][1];
        const auto g = gravity + Vec3(0, f_smooth, 0);
        EXPECT_TRUE(SimTK::Test::numericallyEqual(v, g, 3, TOL)) << "Expected: " << g << " but got: " << v;

        const auto pe = hc_smooth.calcPotentialEnergyContribution(state);
        const auto expectedPE = (2. / 5.) * f_smooth * depth;
        EXPECT_NEAR(pe, expectedPE, TOL);
    }
    ...
    for (Real height = radius + 0.2; height > 0; height -= 0.1) {
        ...
        for (Real v = -1.0; v <= 1.0; v += 0.1) {
            ...
            Real f_smooth = f * (1. / 2. + (1. / 2.) * std::tanh(bv * (v + (2. / (3. * dissipation)))));

            const auto forces =
                system.getRigidBodyForces(state, Stage::Dynamics)[sphere.getMobilizedBodyIndex()][1];
            const auto g = gravity + Vec3(0, f_smooth, 0);
            EXPECT_TRUE(SimTK::Test::numericallyEqual(forces, g, 3, TOL))
                << "Expected: " << g << " but got: " << forces;
        }
    }
    ...
    for (Real height = radius + 0.2; height > 0; height -= 0.1) {
        ...
        for (Real v = -1.0; v <= 1.0; v += 0.1) {
            ...
            Real vrel = vslip / vt;
            Real ff_smooth_scalar =
                fh_smooth * (std::min(vrel, Real(1)) * (ud + 2 * (us - ud) / (1 + vrel * vrel)) + uv * vslip);
            ...
            sphere.applyForceToBodyPoint(state, contactPointInSphere, totalForceOnSphere, expectedForce);
            SpatialVec actualForce =
                system.getRigidBodyForces(state, Stage::Dynamics)[sphere.getMobilizedBodyIndex()];

            const auto actualForce0 = actualForce[0];
            const auto expectedForce0 = expectedForce[sphere.getMobilizedBodyIndex()][0];
            EXPECT_TRUE(SimTK::Test::numericallyEqual(actualForce0, expectedForce0, 3, TOL))
                << "Expected: " << expectedForce0 << " but got: " << actualForce0;

            const auto actualForce1 = actualForce[1];
            const auto expectedForce1 = expectedForce[sphere.getMobilizedBodyIndex()][1];
            EXPECT_TRUE(SimTK::Test::numericallyEqual(actualForce1, expectedForce1, 3, TOL))
                << "Expected: " << expectedForce1 << " but got: " << actualForce1;
        }
    }
}
