#include <gtest/gtest.h>
#include <iostream>
#include <map>

#include "SimTKcommon/internal/SystemGuts.h"

#include "SimTKcommon.h"
#include "Util.hpp"
using std::cout;
using std::endl;

using namespace SimTK;

namespace SimTK {
using EventRegistry = std::map<EventId, SubsystemIndex>;
std::ostream& operator<<(std::ostream& o, const EventRegistry&) {
    return o;
}
} // namespace SimTK

class SystemSubsystemGuts : public Subsystem::Guts {
    public:
    SystemSubsystemGuts() = default;

    [[nodiscard]] const EventRegistry& getEventRegistry(const State& state) const {
        EXPECT_GE(eventRegistry, 0);
        return Value<EventRegistry>::downcast(getCacheEntry(state, eventRegistry));
    }
    [[nodiscard]] auto updEventRegistry(const State& state) const -> EventRegistry& {
        EXPECT_GE(eventRegistry, 0);
        return Value<EventRegistry>::updDowncast(updCacheEntry(state, eventRegistry));
    }

    // implementations of Subsystem::Guts virtuals
    auto cloneImpl() const -> SystemSubsystemGuts* override {
        return new SystemSubsystemGuts(*this);
    }
    auto realizeSubsystemTopologyImpl(State& state) const -> int override {
        eventRegistry = allocateCacheEntry(state, Stage::Instance, new Value<EventRegistry>());
        return 0;
    }

    private:
    mutable CacheEntryIndex eventRegistry;
};


class SystemSubsystem : public Subsystem {
    public:
    SystemSubsystem() {
        adoptSubsystemGuts(new SystemSubsystemGuts());
    }

    void registerEventsToSubsystem(const State& state,
                                   const Subsystem::Guts& sub,
                                   EventId start,
                                   int nEvents) const {
        EventRegistry& eventRegistry = getGuts().updEventRegistry(state);
        SubsystemIndex subsystemIndex = sub.getMySubsystemIndex();
        for (int i = start; i < start + nEvents; ++i) {
            eventRegistry[EventId(i)] = subsystemIndex;
        }
    }

    [[nodiscard]] const EventRegistry& getEventRegistry(const State& s) const {
        return getGuts().getEventRegistry(s);
    }

    private:
    [[nodiscard]] const SystemSubsystemGuts& getGuts() const {
        return dynamic_cast<const SystemSubsystemGuts&>(getSubsystemGuts());
    }
    [[nodiscard]] SystemSubsystemGuts& updGuts() {
        return dynamic_cast<SystemSubsystemGuts&>(updSubsystemGuts());
    }
};


class TestSystemGuts : public System::Guts {
    public:
    [[nodiscard]] const SystemSubsystem& getSystemSubsystem() const {
        return syssub;
    }
    [[nodiscard]] SystemSubsystem& updSystemSubsystem() {
        return syssub;
    }

    // implementations of System::Guts virtuals
    [[nodiscard]] auto cloneImpl() const -> TestSystemGuts* override {
        return new TestSystemGuts(*this);
    }

    auto prescribeQImpl(State& state) const -> bool override {
        return false;
    }
    auto prescribeUImpl(State& state) const -> bool override {
        return false;
    }
    void projectQImpl(State& state,
                      Vector& yerrest,
                      const ProjectOptions& opts,
                      ProjectResults& result) const override {
        result.setExitStatus(ProjectResults::Succeeded);
    }
    void projectUImpl(State& state,
                      Vector& yerrest,
                      const ProjectOptions& opts,
                      ProjectResults& result) const override {
        result.setExitStatus(ProjectResults::Succeeded);
    }
    void handleEventsImpl(State& state,
                          Event::Cause cause,
                          const Array_<EventId>& eventIds,
                          const HandleEventsOptions& options,
                          HandleEventsResults& results) const override {
        cout << "handleEventsImpl t=" << state.getTime() << " cause=" << Event::getCauseName(cause) << endl;

        if (eventIds.empty()) {
            for (SubsystemIndex sx(0); sx < getNumSubsystems(); ++sx) {
                const Subsystem& sub = getSubsystem(sx);
                sub.getSubsystemGuts().handleEvents(state, cause, eventIds, options, results);
                if (results.getExitStatus() == HandleEventsResults::Failed) {
                    break;
                }
            }
            return;
        }

        // If there are EventIds, dole them out to the owning subsystem.
        const EventRegistry& registry = getSystemSubsystem().getEventRegistry(state);

        std::map<SubsystemIndex, Array_<EventId>> eventsPerSub;
        for (EventId eid(0); eid < eventIds.size(); ++eid) {
            eventsPerSub[registry.find(eid)->second].push_back(eid);
        }

        for (const auto& entry : eventsPerSub) {
            const Subsystem& sub = getSubsystem(entry.first);
            sub.getSubsystemGuts().handleEvents(state, cause, entry.second, options, results);
            if (results.getExitStatus() == HandleEventsResults::Failed) {
                break;
            }
        }
    }

    [[nodiscard]] auto reportEventsImpl(const State& state,
                                        Event::Cause cause,
                                        const Array_<EventId>& eventIds) const -> int override {
        cout << "reportEventsImpl t=" << state.getTime() << " cause=" << Event::getCauseName(cause) << endl;
        return 0;
    }

    private:
    SystemSubsystem syssub;
};

class TestSystem : public System {
    public:
    TestSystem() {
        adoptSystemGuts(new TestSystemGuts());
        adoptSubsystem(updGuts().updSystemSubsystem());
    }

    [[nodiscard]] auto getSystemSubsystem() const -> const SystemSubsystem& {
        return getGuts().getSystemSubsystem();
    }
    auto updSystemSubsystem() -> SystemSubsystem& {
        return updGuts().updSystemSubsystem();
    }

    void
    registerEventsToSubsystem(const State& s, const Subsystem::Guts& sub, EventId start, int nEvents) const {
        const SystemSubsystem& syssub = getGuts().getSystemSubsystem();
        syssub.registerEventsToSubsystem(s, sub, start, nEvents);
    }

    static auto getAs(const System& sys) -> const TestSystem& {
        assert(dynamic_cast<const Guts*>(&sys.getSystemGuts()));
        return static_cast<const TestSystem&>(sys);
    }
    static auto updAs(System& sys) -> TestSystem& {
        assert(dynamic_cast<const Guts*>(&sys.getSystemGuts()));
        return static_cast<TestSystem&>(sys);
    }

    private:
    [[nodiscard]] auto getGuts() const -> const TestSystemGuts& {
        return dynamic_cast<const TestSystemGuts&>(getSystemGuts());
    }
    [[nodiscard]] auto updGuts() -> TestSystemGuts& {
        return dynamic_cast<TestSystemGuts&>(updSystemGuts());
    }
};


class TestSubsystemGuts : public Subsystem::Guts {
    struct StateVars {
        QIndex myQs;
        UIndex myUs;
    };
    struct CacheEntries {
        CacheEntryIndex qSumCacheIx, uSumCacheIx;
        EventTriggerByStageIndex timeTriggerIx, velTriggerIx;
    };

    public:
    TestSubsystemGuts() {
    }

    [[nodiscard]] auto getQ3(const State& state) const -> const Vec3& {
        return Vec3::getAs(&getQ(state)[getStateVars(state).myQs]);
    }
    [[nodiscard]] auto getU3(const State& state) const -> const Vec3& {
        return Vec3::getAs(&getU(state)[getStateVars(state).myQs]);
    }
    [[nodiscard]] auto getQDot3(const State& state) const -> const Vec3& {
        return Vec3::getAs(&getQDot(state)[getStateVars(state).myQs]);
    }
    [[nodiscard]] auto getUDot3(const State& state) const -> const Vec3& {
        return Vec3::getAs(&getUDot(state)[getStateVars(state).myUs]);
    }
    [[nodiscard]] auto getQDotDot3(const State& state) const -> const Vec3& {
        return Vec3::getAs(&getQDotDot(state)[getStateVars(state).myQs]);
    }
    [[nodiscard]] auto getQSum(const State& state) const -> Real {
        return Value<Real>::downcast(getCacheEntry(state, getCacheEntries(state).qSumCacheIx));
    }
    [[nodiscard]] auto getUSum(const State& state) const -> Real {
        return Value<Real>::downcast(getCacheEntry(state, getCacheEntries(state).uSumCacheIx));
    }

    auto getTimeTrigger1(const State& state) const -> Real {
        return getEventTriggersByStage(state, Stage::Time)[getCacheEntries(state).timeTriggerIx];
    }
    auto getTimeTrigger2(const State& state) const -> Real {
        return getEventTriggersByStage(state, Stage::Time)[getCacheEntries(state).timeTriggerIx + 1];
    }
    auto getVelTrigger(const State& state) const -> Real {
        return getEventTriggersByStage(state, Stage::Velocity)[getCacheEntries(state).velTriggerIx];
    }

    auto updQ3(State& state) const -> Vec3& {
        return Vec3::updAs(&updQ(state)[getStateVars(state).myQs]);
    }
    auto updU3(State& state) const -> Vec3& {
        return Vec3::updAs(&updU(state)[getStateVars(state).myUs]);
    }

    // implementations of Subsystem::Guts virtuals
    TestSubsystemGuts* cloneImpl() const override {
        return new TestSubsystemGuts(*this);
    }

    auto realizeSubsystemTopologyImpl(State& state) const -> int override {
        myStateVars = allocateCacheEntry(state, Stage::Model, new Value<StateVars>());
        myCacheEntries = allocateCacheEntry(state, Stage::Instance, new Value<CacheEntries>());
        return 0;
    }

    auto realizeSubsystemModelImpl(State& state) const -> int override {
        StateVars& vars = updStateVars(state);
        vars.myQs = allocateQ(state, Vector(Vec3(0)));
        vars.myUs = allocateU(state, Vector(Vec3(0)));
        return 0;
    }

    auto realizeSubsystemInstanceImpl(const State& state) const -> int override {
        CacheEntries& cache = updCacheEntries(state);
        cache.qSumCacheIx = allocateCacheEntry(state, Stage::Position, new Value<Real>(0));
        cache.uSumCacheIx = allocateCacheEntry(state, Stage::Velocity, new Value<Real>(0));
        cache.timeTriggerIx = allocateEventTriggersByStage(state, Stage::Time, 2);
        cache.velTriggerIx = allocateEventTriggersByStage(state, Stage::Velocity, 1);

        getTestSystem().registerEventsToSubsystem(state, *this, EventId(cache.timeTriggerIx), 2);
        getTestSystem().registerEventsToSubsystem(state, *this, EventId(cache.velTriggerIx), 1);
        return 0;
    }

    auto realizeSubsystemTimeImpl(const State& state) const -> int override {
        const Real TriggerTime1 = .6789;
        const Real TriggerTime2 = 1.234;
        updTimeTrigger1(state) = state.getTime() - TriggerTime1;
        updTimeTrigger2(state) = state.getTime() - TriggerTime2;
        return 0;
    }

    auto realizeSubsystemPositionImpl(const State& state) const -> int override {
        updQSum(state) = sum(getQ3(state));
        return 0;
    }

    auto realizeSubsystemVelocityImpl(const State& state) const -> int override {
        const Real TriggerUSum = 5;
        updQDot3(state) = getU3(state);
        const Real usum = updUSum(state) = sum(getU3(state));
        updVelTrigger(state) = usum - TriggerUSum;
        return 0;
    }

    auto realizeSubsystemAccelerationImpl(const State& state) const -> int override {
        updQDotDot3(state) = updUDot3(state) = Vec3(1, 2, 3);
        return 0;
    }

    void handleEventsImpl(State& state,
                          Event::Cause cause,
                          const Array_<EventId>& eventIds,
                          const HandleEventsOptions& options,
                          HandleEventsResults& results) const override {
        cout << "**** TestSubsystem::handleEventsImpl t=" << state.getTime()
             << " acc=" << options.getAccuracy() << " eventIds=";
        for (unsigned i = 0; i < eventIds.size(); ++i) {
            cout << " " << eventIds[i];
        }
        cout << " ****" << endl;

        // Pretend we changed a position to test lowestModifiedStage
        // calculation. Try to hide our duplicity by realizing it again.
        state.invalidateAllCacheAtOrAbove(Stage::Position);
        getSystem().realize(state, Stage::Velocity);
        results.setExitStatus(HandleEventsResults::Succeeded);
    }

    void
    reportEventsImpl(const State& state, Event::Cause cause, const Array_<EventId>& eventIds) const override {
    }

    private:
    auto updQDot3(const State& state) const -> Vec3& {
        return Vec3::updAs(&updQDot(state)[getStateVars(state).myQs]);
    }
    auto updUDot3(const State& state) const -> Vec3& {
        return Vec3::updAs(&updUDot(state)[getStateVars(state).myUs]);
    }
    auto updQDotDot3(const State& state) const -> Vec3& {
        return Vec3::updAs(&updQDotDot(state)[getStateVars(state).myQs]);
    }
    auto updQSum(const State& state) const -> Real& {
        return Value<Real>::updDowncast(updCacheEntry(state, getCacheEntries(state).qSumCacheIx));
    }
    auto updUSum(const State& state) const -> Real& {
        return Value<Real>::updDowncast(updCacheEntry(state, getCacheEntries(state).uSumCacheIx));
    }
    auto updTimeTrigger1(const State& state) const -> Real& {
        return updEventTriggersByStage(state, Stage::Time)[getCacheEntries(state).timeTriggerIx];
    }
    auto updTimeTrigger2(const State& state) const -> Real& {
        return updEventTriggersByStage(state, Stage::Time)[getCacheEntries(state).timeTriggerIx + 1];
    }
    auto updVelTrigger(const State& state) const -> Real& {
        return updEventTriggersByStage(state, Stage::Velocity)[getCacheEntries(state).velTriggerIx];
    }

    auto getStateVars(const State& state) const -> const StateVars& {
        assert(myStateVars >= 0);
        return Value<StateVars>::downcast(getCacheEntry(state, myStateVars));
    }
    auto getCacheEntries(const State& state) const -> const CacheEntries& {
        assert(myCacheEntries >= 0);
        return Value<CacheEntries>::downcast(getCacheEntry(state, myCacheEntries));
    }
    auto updStateVars(const State& state) const -> StateVars& {
        assert(myStateVars >= 0);
        return Value<StateVars>::updDowncast(updCacheEntry(state, myStateVars));
    }
    auto updCacheEntries(const State& state) const -> CacheEntries& {
        assert(myCacheEntries >= 0);
        return Value<CacheEntries>::updDowncast(updCacheEntry(state, myCacheEntries));
    }

    auto getTestSystem() const -> const TestSystem& {
        return TestSystem::getAs(getSystem());
    }
    auto updTestSystem() -> TestSystem& {
        return TestSystem::updAs(updSystem());
    }

    // TOPOLOGY STATE VARIABLES //
    Array_<EventHandler*> eventHandlers;
    mutable Array_<EventReporter*> eventReporters;

    // TOPOLOGY CACHE //
    mutable CacheEntryIndex myStateVars;
    mutable CacheEntryIndex myCacheEntries;
};

// This Subsystem has 3 q's and 3 u's of its own, as well as whatever State
// variables its Measures require.
class TestSubsystem : public Subsystem {
    public:
    TestSubsystem(System& sys) {
        adoptSubsystemGuts(new TestSubsystemGuts());
        sys.adoptSubsystem(*this);
    }

    [[nodiscard]] auto getQSum(const State& state) const -> Real {
        return getGuts().getQSum(state);
    }
    [[nodiscard]] auto getUSum(const State& state) const -> Real {
        return getGuts().getUSum(state);
    }

    private:
    [[nodiscard]] auto getGuts() const -> const TestSubsystemGuts& {
        return dynamic_cast<const TestSubsystemGuts&>(getSubsystemGuts());
    }
    auto updGuts() -> TestSubsystemGuts& {
        return dynamic_cast<TestSubsystemGuts&>(updSubsystemGuts());
    }
};

// Find the event triggers at a particular stage that changed sign since
// they were last recorded in events0.
static void findEvents(const State& state, Stage g, const Vector& triggers0, Array_<EventId>& triggered) {
    const int n = state.getNEventTriggersByStage(g);
    const int start = state.getEventTriggerStartByStage(g); // location within triggers0 Vector

    const Vector& stageTriggers = state.getEventTriggersByStage(g);

    triggered.clear();
    for (int i = 0; i < n; ++i) {
        const EventId allStageId = EventId(start + i);
        if (sign(triggers0[allStageId]) != sign(stageTriggers[i])) {
            triggered.push_back(allStageId);
        }
    }
}

static Real accuracy = 1e-6;
static Real timescale;

static auto handleEvents(const System& sys, State& state, Stage g, const Array_<EventId>& triggered) -> bool {
    if (triggered.empty()) {
        return false;
    }

    cout << "==> Handling " << triggered.size() << " events at Stage " << g << ":";
    for (unsigned i = 0; i < triggered.size(); ++i) {
        cout << " " << triggered[i];
    }
    cout << endl;

    bool shouldTerminate = false;
    HandleEventsOptions options(accuracy);
    HandleEventsResults results;

    Array_<StageVersion> stageVersions;
    state.getSystemStageVersions(stageVersions);
    cout << "BEFORE handling stage versions=\n";
    cout << stageVersions << "\n";
    sys.handleEvents(state, Event::Cause::Triggered, triggered, options, results);
    state.getSystemStageVersions(stageVersions);
    cout << "AFTER handling stage versions=\n";
    cout << stageVersions << "\n";
    cout << "Results lowestStage=" << results.getLowestModifiedStage() << "\n";

    if (results.getExitStatus() == HandleEventsResults::ShouldTerminate) {
        cout << "==> Event at Stage " << g << " requested termination at t=" << state.getTime() << endl;
        shouldTerminate = true;
    }

    return shouldTerminate;
}

template <class T>
class MySinCos : public Measure_<T> {
    public:
    SimTK_MEASURE_HANDLE_PREAMBLE(MySinCos, Measure_<T>);

    SimTK_MEASURE_HANDLE_POSTSCRIPT(MySinCos, Measure_<T>);
};


template <class T>
class MySinCos<T>::Implementation : public Measure_<T>::Implementation {
    public:
    Implementation()
        : Measure_<T>::Implementation(T(Vec2(0)), 1) {
    }

    // Default copy constructor, destructor, copy assignment are fine.

    // Implementations of virtual methods.
    [[nodiscard]] auto cloneVirtual() const -> Implementation* override {
        return new Implementation(*this);
    }
    [[nodiscard]] auto getNumTimeDerivativesVirtual() const -> int override {
        return 0;
    }
    [[nodiscard]] auto getDependsOnStageVirtual(int order) const -> Stage override {
        return Stage::Time;
    }

    void calcCachedValueVirtual(const State& state, int derivOrder, T& value) const override {
        SimTK_ASSERT1_ALWAYS(derivOrder == 0,
                             "MySinCos::Implementation::calcCachedValueVirtual():"
                             " derivOrder %d seen but only 0 allowed.",
                             derivOrder);
        value[0] = std::sin(state.getTime());
        value[1] = std::cos(state.getTime());
    }
};

template <class T>
class MyRealMeasure : public Measure_<T> {
    public:
    SimTK_MEASURE_HANDLE_PREAMBLE(MyRealMeasure, Measure);

    SimTK_MEASURE_HANDLE_POSTSCRIPT(MyRealMeasure, Measure);
};

template <class T>
class MyRealMeasure<T>::Implementation : public Measure_<T>::Implementation {
    public:
    [[nodiscard]] auto cloneVirtual() const -> Implementation* override {
        return new Implementation(*this);
    }
    [[nodiscard]] auto getNumTimeDerivativesVirtual() const -> int override {
        return 0;
    }
    [[nodiscard]] auto getDependsOnStageVirtual(int order) const -> Stage override {
        return Stage::Time;
    }
};


// void testOne() {
//     TestSystem sys;
//     TestSubsystem subsys(sys);

//     // Add a Result measure to the system subsystem. This depends on
//     // Position stage and invalidates Dynamics and later stages.
//     Measure_<Vector>::Result vectorResult(sys.updSystemSubsystem(), Stage::Position, Stage::Dynamics);

//     MeasureIndex vectorResultIx = vectorResult.getSubsystemMeasureIndex();
//     cout << "vectorResult index=" << vectorResultIx << endl;

//     Measure_<Vector>::Result myVecRes =
//         Measure_<Vector>::Result::getAs(sys.updSystemSubsystem().getMeasure(vectorResultIx));

//     Measure::Result result(sys.updSystemSubsystem(), Stage::Time, Stage::Position);
//     Measure::Result autoResult(sys.updSystemSubsystem(), Stage::Time, Stage::Position);
//     autoResult.setIsPresumedValidAtDependsOnStage(true);

//     Measure::Zero zero(subsys);
//     Measure::Constant three(subsys, 3);
//     Measure_<Vec3>::Constant v3const(subsys, Vec3(1, 2, 3));
//     Measure::Sinusoid cos2pit(subsys, 1, 2 * Pi, Pi / 2);

//     // Integrate the cos(2pi*t) measure with IC=0; should give sin(2pi*t)/2pi.
//     Measure::Integrate sin2pitOver2pi(subsys, cos2pit, zero);

//     // These two compute -cos(t), sin(t) by integrating sin(t), cos(t) with
//     // initial conditions -1,0.
//     Measure_<Vec2>::Constant cossinInit(subsys, Vec2(-1, 0));
//     MySinCos<Vec2> mysincos(subsys);
//     Measure_<Vec2>::Integrate cossin(subsys, mysincos, cossinInit);

//     Measure_<Vector>::Constant vcossinInit(subsys, Vector(Vec2(-1, 0)));
//     MySinCos<Vector> vmysincos(subsys);
//     Measure_<Vector>::Integrate vcossin(subsys, vmysincos, vcossinInit, Vector(2, Zero));
//     Measure_<Vector>::Delay vcossin_delaypt1(subsys, vcossin, .1);

//     Measure_<Real>::Minimum minCos2pit(subsys, cos2pit);
//     Measure_<Real>::Maximum maxCos2pit(subsys, cos2pit);
//     Measure_<Real>::MinAbs minAbsCos2pit(subsys, cos2pit);
//     Measure_<Real>::MaxAbs maxAbsCos2pit(subsys, cos2pit);

//     Measure::Differentiate dInteg(subsys, sin2pitOver2pi);
//     dInteg.setForceUseApproximation(true);

//     Measure::Time tMeasure;
//     Measure::Time tSubMeas(subsys);

//     Measure::Delay tDelayed(subsys, tSubMeas, 0.01);

//     Measure::Scale t1000(subsys, 1000, tSubMeas);

//     Measure::Variable mv(subsys, Stage::Position, 29);
//     cout << "mv def value=" << mv.getDefaultValue() << endl;
//     mv.setDefaultValue(-19);
//     cout << "mv def value now=" << mv.getDefaultValue() << endl;

//     cout << "Measure::Zero=" << Measure::Zero().getValue(State()) << endl;
//     cout << "Measure::One=" << Measure::One().getValue(State()) << endl;

//     Measure::Plus vplus(subsys, mv, cos2pit);
//     Measure::Minus vminus(subsys, mv, cos2pit);

//     Measure::Plus vplus2;
//     vplus2.deepAssign(vplus);

//     Measure m;
//     m = cos2pit;


//     cout << "vplus ref count=" << vplus.getRefCount() << endl;
//     cout << "vplus2 ref count=" << vplus2.getRefCount() << endl;


//     State state = sys.realizeTopology();
//     cout << "sys topo version=" << sys.getSystemTopologyCacheVersion() << "\n";
//     cout << "state topo version=" << state.getSystemTopologyStageVersion() << "\n";
//     sys.invalidateSystemTopologyCache();
//     sys.realizeTopology();
//     cout << "sys topo version=" << sys.getSystemTopologyCacheVersion() << "\n";
//     // Use sneaky loophole since we know state is still good.
//     state.setSystemTopologyStageVersion(sys.getSystemTopologyCacheVersion());
//     sys.realizeModel(state);

//     m = zero;
//     cout << "m=" << m.getValue(state) << endl;


//     cout << "uWeights=" << state.getUWeights() << "\n";
//     cout << "zWeights=" << state.getZWeights() << "\n";

//     state.updUWeights()[1] = 9;
//     state.updZWeights() = 21;

//     cout << "uWeights=" << state.getUWeights() << "\n";
//     cout << "zWeights=" << state.getZWeights() << "\n";

//     sys.realize(state, Stage::Instance);

//     cout << "qerrWeights=" << state.getQErrWeights() << "\n";
//     cout << "uerrWeights=" << state.getUErrWeights() << "\n";

//     State dupState = state;
//     cout << "dup uWeights=" << state.getUWeights() << "\n";
//     cout << "dup zWeights=" << state.getZWeights() << "\n";
//     cout << "dup qerrWeights=" << state.getQErrWeights() << "\n";
//     cout << "dup uerrWeights=" << state.getUErrWeights() << "\n";

//     // Allocate vectorResult and initialize it. (Can't mark it valid yet.)
//     vectorResult.updValue(state).resize(3);
//     vectorResult.updValue(state) = Vector(Vec3(1, 2, 3));

//     result.updValue(state) = 1.234;
//     autoResult.updValue(state) = 4.321;


//     state.setTime(1.234);
//     sys.realize(state, Stage::Time);
//     cout << "Initially, tMeasure=" << tMeasure.getValue(state) << " tSubMeas=" << tSubMeas.getValue(state)
//          << " 1000*tMeasure=" << t1000.getValue(state) << endl;

//     Measure_<Mat22>::One m22Ident;
//     cout << "Measure_<Mat22>::One=" << m22Ident.getValue(state) << endl;

//     cout << "mv after realizeTopo=" << mv.getValue(state) << endl;

//     cout << "cossinInit=" << cossinInit.getValue(state) << endl;


//     State s2, s3;
//     s2 = state; // new copies of variables
//     s2 = state; // should do only assignments w/o heap allocation

//     // Explicit midpoint steps.
//     const Real h = .001;
//     const int nSteps = 2000;
//     const int outputInterval = 100;
//     state.setTime(0);

//     EXPECT_EQ(state.getTime(), 0);


//     // initialize()
//     sys.realize(state, Stage::Position);
//     cout << "mv+cos2pit=" << vplus.getValue(state) << endl;
//     cout << "mv-cos2pit=" << vminus.getValue(state) << endl;

//     cout << "Sys stage after realize(Pos):" << state.getSystemStage().getName() << endl;

//     mv.setValue(state, 1.234);

//     cout << "Sys stage after mv=1.234:" << state.getSystemStage().getName() << endl;

//     cout << "mv is now=" << mv.getValue(state) << endl;

//     sys.realize(state, Stage::Position);

//     cout << "Realized Position:\n";
//     vectorResult.markAsValid(state);
//     cout << "vectorResult=" << vectorResult.getValue(state) << endl;
//     cout << "myVecRes=" << myVecRes.getValue(state) << endl;
//     result.markAsValid(state);
//     cout << "result=" << result.getValue(state) << endl;
//     // Shouldn't need to mark this one.
//     cout << "autoResult=" << autoResult.getValue(state) << endl;

//     // Fill in statics above.
//     timescale = sys.getDefaultTimeScale();

//     sys.realize(state, Stage::Acceleration);

//     cout << "Now stage=" << state.getSystemStage() << endl;
//     vectorResult.setValue(state, Vector(5, 9));
//     cout << "After vectorResult.setValue(), vectorResult=" << vectorResult.getValue(state) << endl;
//     cout << "... but stage=" << state.getSystemStage() << endl;

//     cossin.setValue(state, cossinInit.getValue(state));
//     vcossin.setValue(state, vcossinInit.getValue(state));

//     // Handler is allowed to throw an exception if it fails since we don't
//     // have a way to recover.
//     HandleEventsOptions handleOpts;
//     HandleEventsResults results;
//     sys.handleEvents(state, Event::Cause::Initialization, Array_<EventId>(), handleOpts, results);
//     SimTK_ERRCHK_ALWAYS(results.getExitStatus() != HandleEventsResults::ShouldTerminate,
//                         "Integrator::initialize()",
//                         "An initialization event handler requested termination.");

//     sys.realize(state, Stage::Acceleration);
//     state.autoUpdateDiscreteVariables(); // ??

//     for (int i = 0; i <= nSteps; ++i) {
//         if (i % outputInterval == 0) {
//             sys.realize(state, Stage::Report);
//             cout << "\ntMeasure=" << tMeasure.getValue(state)
//                  << " d/dt tMeasure=" << tMeasure.getValue(state, 1)
//                  << " d3/dt3 tMeasure=" << tMeasure.getValue(state, 3)
//                  << " 1000*tSubMeas=" << t1000.getValue(state) << " t=" << state.getTime() << endl;
//             cout << " tDelayed=" << tDelayed.getValue(state) << endl;
//             cout << "q=" << state.getQ() << " u=" << state.getU() << endl;
//             cout << "qSum=" << subsys.getQSum(state) << " uSum=" << subsys.getUSum(state) << endl;
//             cout << "three=" << three.getValue(state) << " v3const=" << v3const.getValue(state) << endl;
//             cout << "cos2pit=" << cos2pit.getValue(state)
//                  << " cos(2pi*t)=" << std::cos(2 * Pi * state.getTime()) << endl;
//             cout << "Min(cos2pit)=" << minCos2pit.getValue(state)
//                  << " @t=" << minCos2pit.getTimeOfExtremeValue(state) << endl;
//             cout << "Max(cos2pit)=" << maxCos2pit.getValue(state)
//                  << " @t=" << maxCos2pit.getTimeOfExtremeValue(state) << endl;
//             cout << "MinAbs(cos2pit)=" << minAbsCos2pit.getValue(state)
//                  << " @t=" << minAbsCos2pit.getTimeOfExtremeValue(state) << endl;
//             cout << "MaxAbs(cos2pit)=" << maxAbsCos2pit.getValue(state)
//                  << " @t=" << maxAbsCos2pit.getTimeOfExtremeValue(state) << endl;
//             cout << "sin2pitOver2pi=" << sin2pitOver2pi.getValue(state)
//                  << " sin(2pi*t)/2pi=" << std::sin(2 * Pi * state.getTime()) / (2 * Pi) << endl;
//             cout << "d/dt sin2pitOver2pi=" << sin2pitOver2pi.getValue(state, 1) << endl;
//             cout << "dInteg=" << dInteg.getValue(state) << endl;
//             cout << "cossin=" << cossin.getValue(state) << "\n";
//             cout << "vcossin=" << vcossin.getValue(state) << "\n";
//             cout << "vcossin delay .1=" << vcossin_delaypt1.getValue(state) << "\n";
//         }

//         if (i == nSteps) {
//             break;
//         }

//         const Real h2 = h / 2;
//         const Vector ydot0 = state.getYDot();
//         const Vector triggers0 = state.getEventTriggers();
//         Array_<EventId> triggered;

//         // Commit the values for the discrete variable updates calculated
//         // at the end of the previous step. This includes both explicitly
//         // discrete variables and continuous variables which are defined
//         // by algebraic rather than differential equations, such as
//         // prescribed motions. This requires that all calculations have
//         // been performed already using the *updated* values, *not* the
//         // state values; that permits us to perform this update without
//         // invalidating any cache entries.
//         state.autoUpdateDiscreteVariables();

//         // First integrator stage: unconstrained continuous system only.
//         state.updY() += h2 * ydot0;
//         state.updTime() += h2;
//         sys.realize(state, Stage::Time);
//         sys.prescribeQ(state);
//         sys.realize(state, Stage::Position);
//         sys.prescribeU(state);
//         sys.realize(state);

//         // Second (final) integrator stage.
//         // 1. Unconstrained continuous system.
//         const Vector& ydot = state.getYDot();
//         state.updY() += h2 * ydot; // that is, y = y0 + h*(ydot0+ydot)/2
//         state.updTime() += h2;
//         sys.realize(state, Stage::Time);
//         sys.prescribeQ(state);

//         // 2. Deal with time-dependent events.
//         findEvents(state, Stage::Time, triggers0, triggered);
//         if (handleEvents(sys, state, Stage::Time, triggered)) {
//             break;
//         }

//         sys.realize(state, Stage::Position);
//         sys.prescribeU(state);

//         // 3a. Project position-dependent constraints.
//         Vector temp;
//         ProjectOptions opts(accuracy);
//         ProjectResults results;
//         sys.projectQ(state, temp, opts, results);

//         // 3b. Handle position-dependent events.
//         findEvents(state, Stage::Position, triggers0, triggered);
//         if (handleEvents(sys, state, Stage::Position, triggered)) {
//             break;
//         }

//         // 4a. Project velocity-dependent constraints.
//         sys.realize(state, Stage::Velocity);
//         sys.projectU(state, temp, opts, results);

//         // 4b. Handle velocity-dependent events.
//         findEvents(state, Stage::Velocity, triggers0, triggered);
//         if (handleEvents(sys, state, Stage::Velocity, triggered)) {
//             break;
//         }

//         // 5. Handle dynamics- and acceleration-dependent events.
//         sys.realize(state, Stage::Dynamics);
//         findEvents(state, Stage::Dynamics, triggers0, triggered);
//         if (handleEvents(sys, state, Stage::Dynamics, triggered)) {
//             break;
//         }
//         sys.realize(state, Stage::Acceleration);
//         findEvents(state, Stage::Acceleration, triggers0, triggered);
//         if (handleEvents(sys, state, Stage::Acceleration, triggered)) {
//             break;
//         }

//         // Ensure State is realized through Acceleration Stage.
//         sys.realize(state, Stage::Acceleration);
//     }
// }


TEST(SimTKCommon_Simulation_MeasuresAndSubsystemConstruction, ConstructsMeasuresAndSubsystemGraph) {
    TestSystem sys;
    TestSubsystem subsys(sys);

    Measure_<Vector>::Result vectorResult(sys.updSystemSubsystem(), Stage::Position, Stage::Dynamics);

    MeasureIndex vectorResultIx = vectorResult.getSubsystemMeasureIndex();

    Measure_<Vector>::Result myVecRes =
        Measure_<Vector>::Result::getAs(sys.updSystemSubsystem().getMeasure(vectorResultIx));

    Measure::Result result(sys.updSystemSubsystem(), Stage::Time, Stage::Position);

    Measure::Result autoResult(sys.updSystemSubsystem(), Stage::Time, Stage::Position);

    autoResult.setIsPresumedValidAtDependsOnStage(true);

    Measure::Zero zero(subsys);
    Measure::Constant three(subsys, 3);
    Measure_<Vec3>::Constant v3const(subsys, Vec3(1, 2, 3));
    Measure::Sinusoid cos2pit(subsys, 1, 2 * Pi, Pi / 2);

    Measure::Integrate sin2pitOver2pi(subsys, cos2pit, zero);

    Measure_<Vec2>::Constant cossinInit(subsys, Vec2(-1, 0));
    MySinCos<Vec2> mysincos(subsys);
    Measure_<Vec2>::Integrate cossin(subsys, mysincos, cossinInit);

    Measure_<Vector>::Constant vcossinInit(subsys, Vector(Vec2(-1, 0)));
    MySinCos<Vector> vmysincos(subsys);
    Measure_<Vector>::Integrate vcossin(subsys, vmysincos, vcossinInit, Vector(2, Zero));

    Measure_<Vector>::Delay vcossin_delaypt1(subsys, vcossin, .1);

    Measure_<Real>::Minimum minCos2pit(subsys, cos2pit);
    Measure_<Real>::Maximum maxCos2pit(subsys, cos2pit);
    Measure_<Real>::MinAbs minAbsCos2pit(subsys, cos2pit);
    Measure_<Real>::MaxAbs maxAbsCos2pit(subsys, cos2pit);

    Measure::Differentiate dInteg(subsys, sin2pitOver2pi);
    dInteg.setForceUseApproximation(true);

    Measure::Time tMeasure;
    Measure::Time tSubMeas(subsys);

    Measure::Delay tDelayed(subsys, tSubMeas, 0.01);
    Measure::Scale t1000(subsys, 1000, tSubMeas);

    Measure::Variable mv(subsys, Stage::Position, 29);
    mv.setDefaultValue(-19);

    Measure::Plus vplus(subsys, mv, cos2pit);
    Measure::Minus vminus(subsys, mv, cos2pit);

    Measure::Plus vplus2;
    vplus2.deepAssign(vplus);

    Measure m;
    m = cos2pit;

    EXPECT_EQ(vplus.getRefCount(), vplus2.getRefCount());
}

TEST(SimTKCommon_Simulation_TopologyAndModelRealization, RealizesTopologyModelAndWeightsCorrectly) {
    TestSystem sys;
    TestSubsystem subsys(sys);

    State state = sys.realizeTopology();

    sys.invalidateSystemTopologyCache();
    sys.realizeTopology();

    state.setSystemTopologyStageVersion(sys.getSystemTopologyCacheVersion());
    sys.realizeModel(state);

    const auto m = Measure::Zero();
    EXPECT_EQ(m.getValue(state), 0);

    state.updUWeights()[1] = 9;
    state.updZWeights() = 21;

    EXPECT_EQ(state.getUWeights()[1], 9) << state.getUWeights();
    for (std::size_t i = 0; i < state.getZWeights().size(); ++i) {
        EXPECT_EQ(state.getZWeights()[i], 21) << state.getZWeights();
    }

    sys.realize(state, Stage::Instance);
}

TEST(SimTKCommon_Simulation_StateCopyAndMeasures, StateCopyPreservesWeightsAndMeasures) {
    TestSystem sys;
    TestSubsystem subsys(sys);

    State state = sys.realizeTopology();
    sys.realizeModel(state);
    sys.realize(state, Stage::Instance);

    State dupState = state;

    EXPECT_TRUE(AssertSimTKEqual("state.getUWeights()",
                                 "dupState.getUWeights()",
                                 state.getUWeights(),
                                 dupState.getUWeights()));
    EXPECT_TRUE(AssertSimTKEqual("state.getZWeights()",
                                 "dupState.getZWeights()",
                                 state.getZWeights(),
                                 dupState.getZWeights()));
}

TEST(SimTKCommon_Simulation_MeasureValuesAndRealization, MeasuresEvaluateCorrectlyAfterRealization) {
    TestSystem sys;
    TestSubsystem subsys(sys);

    Measure_<Vector>::Result vectorResult(sys.updSystemSubsystem(), Stage::Position, Stage::Dynamics);

    Measure::Result result(sys.updSystemSubsystem(), Stage::Time, Stage::Position);

    Measure::Result autoResult(sys.updSystemSubsystem(), Stage::Time, Stage::Position);

    autoResult.setIsPresumedValidAtDependsOnStage(true);

    State state = sys.realizeTopology();
    sys.realizeModel(state);

    vectorResult.updValue(state).resize(3);
    vectorResult.updValue(state) = Vector(Vec3(1, 2, 3));

    result.updValue(state) = 1.234;
    autoResult.updValue(state) = 4.321;

    state.setTime(1.234);
    sys.realize(state, Stage::Time);

    EXPECT_EQ(vectorResult.getValue(state).size(), 3);
    EXPECT_EQ(result.getValue(state), 1.234);
    EXPECT_EQ(autoResult.getValue(state), 4.321);
}

TEST(SimTKCommon_Simulation_InitializationEvents, HandlesInitializationEventsWithoutTermination) {
    TestSystem sys;
    TestSubsystem subsys(sys);

    State state = sys.realizeTopology();
    sys.realizeModel(state);
    sys.realize(state, Stage::Acceleration);

    HandleEventsOptions handleOpts;
    HandleEventsResults results;

    sys.handleEvents(state, Event::Cause::Initialization, Array_<EventId>(), handleOpts, results);

    EXPECT_NE(results.getExitStatus(), HandleEventsResults::ShouldTerminate);
}

TEST(SimTKCommon_Simulation_IntegrationLoop, AdvancesStateAndExecutesFullEventPipeline) {
    TestSystem sys;
    TestSubsystem subsys(sys);

    Measure_<Vector>::Result vectorResult(sys.updSystemSubsystem(), Stage::Position, Stage::Dynamics);

    Measure::Result result(sys.updSystemSubsystem(), Stage::Time, Stage::Position);

    Measure::Result autoResult(sys.updSystemSubsystem(), Stage::Time, Stage::Position);

    autoResult.setIsPresumedValidAtDependsOnStage(true);

    Measure::Zero zero(subsys);
    Measure::Constant three(subsys, 3);
    Measure_<Vec3>::Constant v3const(subsys, Vec3(1, 2, 3));
    Measure::Sinusoid cos2pit(subsys, 1, 2 * Pi, Pi / 2);

    Measure::Integrate sin2pitOver2pi(subsys, cos2pit, zero);

    Measure_<Vec2>::Constant cossinInit(subsys, Vec2(-1, 0));
    MySinCos<Vec2> mysincos(subsys);
    Measure_<Vec2>::Integrate cossin(subsys, mysincos, cossinInit);

    Measure_<Vector>::Constant vcossinInit(subsys, Vector(Vec2(-1, 0)));
    MySinCos<Vector> vmysincos(subsys);
    Measure_<Vector>::Integrate vcossin(subsys, vmysincos, vcossinInit, Vector(2, Zero));

    Measure_<Vector>::Delay vcossin_delaypt1(subsys, vcossin, .1);

    Measure_<Real>::Minimum minCos2pit(subsys, cos2pit);
    Measure_<Real>::Maximum maxCos2pit(subsys, cos2pit);
    Measure_<Real>::MinAbs minAbsCos2pit(subsys, cos2pit);
    Measure_<Real>::MaxAbs maxAbsCos2pit(subsys, cos2pit);

    Measure::Differentiate dInteg(subsys, sin2pitOver2pi);
    dInteg.setForceUseApproximation(true);

    Measure::Time tMeasure;
    Measure::Time tSubMeas(subsys);
    Measure::Delay tDelayed(subsys, tSubMeas, 0.01);
    Measure::Scale t1000(subsys, 1000, tSubMeas);

    Measure::Variable mv(subsys, Stage::Position, 29);
    mv.setDefaultValue(-19);

    Measure::Plus vplus(subsys, mv, cos2pit);
    Measure::Minus vminus(subsys, mv, cos2pit);

    State state = sys.realizeTopology();
    sys.realizeModel(state);

    const Real h = .001;
    const int nSteps = 2000;
    const int outputInterval = 100;

    state.setTime(0);
    EXPECT_EQ(state.getTime(), 0);

    sys.realize(state, Stage::Position);

    mv.setValue(state, 1.234);
    sys.realize(state, Stage::Position);

    sys.realize(state, Stage::Acceleration);

    HandleEventsOptions handleOpts;
    HandleEventsResults results;

    for (int i = 0; i <= nSteps; ++i) {
        if (i == nSteps) {
            break;
        }

        const Real h2 = h / 2;
        const Vector ydot0 = state.getYDot();
        const Vector triggers0 = state.getEventTriggers();
        Array_<EventId> triggered;

        state.autoUpdateDiscreteVariables();

        state.updY() += h2 * ydot0;
        state.updTime() += h2;

        sys.realize(state, Stage::Time);
        sys.prescribeQ(state);
        sys.realize(state, Stage::Position);
        sys.prescribeU(state);
        sys.realize(state);

        const Vector& ydot = state.getYDot();
        state.updY() += h2 * ydot;
        state.updTime() += h2;

        sys.realize(state, Stage::Time);
        sys.prescribeQ(state);

        findEvents(state, Stage::Time, triggers0, triggered);
        if (handleEvents(sys, state, Stage::Time, triggered)) {
            break;
        }

        sys.realize(state, Stage::Position);
        sys.prescribeU(state);

        Vector temp;
        ProjectOptions opts(accuracy);
        ProjectResults resultsProj;

        sys.projectQ(state, temp, opts, resultsProj);

        findEvents(state, Stage::Position, triggers0, triggered);
        if (handleEvents(sys, state, Stage::Position, triggered)) {
            break;
        }

        sys.realize(state, Stage::Velocity);
        sys.projectU(state, temp, opts, resultsProj);

        findEvents(state, Stage::Velocity, triggers0, triggered);
        if (handleEvents(sys, state, Stage::Velocity, triggered)) {
            break;
        }

        sys.realize(state, Stage::Dynamics);
        findEvents(state, Stage::Dynamics, triggers0, triggered);
        if (handleEvents(sys, state, Stage::Dynamics, triggered)) {
            break;
        }

        sys.realize(state, Stage::Acceleration);
        findEvents(state, Stage::Acceleration, triggers0, triggered);
        if (handleEvents(sys, state, Stage::Acceleration, triggered)) {
            break;
        }

        sys.realize(state, Stage::Acceleration);
    }
}
