...
parser.add_argument(
    "--simDirs", default=None, nargs="+", help="Directory with input data files"
)
parser.add_argument(
    "--logDirs", default=None, nargs="+", help="Directory with input data files"
)
parser.add_argument(
    "--molName",
    default=None,
...
parser.add_argument(
    "--FNSeeds", default=None, nargs="+", help="Name root for first input data files"
)
parser.add_argument(
    "--skip_header",
    default=0,
...
parser.add_argument("--stride", default=1, type=int, help="Stride for the read lines.")
parser.add_argument(
    "--nbins",
    default=50,
    type=int,
...
parser.add_argument("--analyze", default=[], nargs="+", help="Decide what to analyze")
...
parser.add_argument(
    ...
    type=int,
    nargs="+",
    help="Atom indices to compute distance for.",
)

parser.add_argument(
    "--trajDiffWin", default=3, type=int, help="Moving difference window."
)
parser.add_argument("--makeplots", default=[], nargs="+", help="Make plots")
parser.add_argument(
    "--savefigs", action="store_true", default=False, help="Save the plot into a file"
)
...
# [[s1 s2 s3], [s4 s5 6]]
seeds = np.array(
    np.array_split(np.array([int(seed) for seed in args.FNSeeds]), nofParamSets)
)
print(seeds)
...
if "traj" in args.analyze:
        ...
        for framei in range(0, data.shape[1]):
            ...
            for simi in paramSetSimsRange:
                ...
                if data[simi, framei] != None:
                    ...
                    cumulativeBias[simi, framei] = (
                        cumulativeMean[simi, framei] - trueValueEst
                    )
            ...
            MSE[paramSeti, framei] = (
                vofm[paramSeti, framei] + (bias[paramSeti, framei]) ** 2
            )
    ...
    for paramSeti in range(nofParamSets):
        expFitFactors[paramSeti] = np.polyfit(
            np.array(range(noNanMSE.shape[1])), np.log(noNanMSE[paramSeti]), 1
        )
    ...
    for paramSeti in range(nofParamSets):
        popt, pcov = curve_fit(
            func, np.array(range(1, noNanMSE.shape[1])), noNanMSE[paramSeti][1:]
        )
        poptpcov[paramSeti] = []
    ...
    print(
        "halfLifeMSE expFactors ratios second/first",
        halfLifeMSE[1] / halfLifeMSE[0],
        (-1.0 * poptpcov[1][0][1]) / (-1.0 * poptpcov[0][0][1]),
    )
    ...
    if args.makeplots:
        ...
        for paramSeti in range(nofParamSets):
            ...
            for simi in range(startSim, nofSimsPerParamSet + startSim):
                ...
                axs[0].set_title(
                    "Running Means " + args.molName + " seed " + str(args.FNSeeds[0])
                )
            ...
            axs[1].plot(
                X,
                func(X, *(poptpcov[paramSeti][0])),
                "r-",
                label="Fitted Curve",
                color=colors[paramSeti],
                linestyle="dashed",
            )
