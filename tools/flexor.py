...
class Flexor:
            ...
            if (edge[0].index in selIx) and (edge[1].index in selIx):
                ...
                if jointType == "cart":
                    flexibilities.append(
                        robosample.BondFlexibility(
                            edge[0].index,
                            edge[1].index,
                            robosample.BondMobility.Translation,
                        )
                    )
                            ...
                            if jointType.capitalize() == "Pin":
                                flexibilities.append(
                                    robosample.BondFlexibility(
                                        edge[0].index,
                                        edge[1].index,
                                        robosample.BondMobility.Torsion,
                                    )
                                )
                            else:
    ...
    def create_from_list(self, bond_list, jointType):
        ...
        for bond in bond_list:
            flexibilities.append(
                robosample.BondFlexibility(bond[0], bond[1], jointType)
            )
        return flexibilities
