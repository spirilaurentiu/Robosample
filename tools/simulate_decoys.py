...
for cluster in clusters:
    ...
    decoy_bonds = stats.chose_decoy_bonds(bond_list, steps=10000)
...
context.getWorld(0).add_sampler(
    sampler, robosample.IntegratorType.OMMVV, thermostat, False
)
for i in range(len(clusters)):
    context.getWorld(i + 1).add_sampler(
        sampler, robosample.IntegratorType.VERLET, thermostat, True
    )
