...
class TrajectoryAnalyzer:
    def __init__(self, topFN, molName, FNSeeds, simDirs, pattern=None):
        ...
        if pattern:
            ...
            self.pattern = ""
        print(
            "TrajectoryAnalyzer init simDirs FNSeeds pattern",
            self.simDirs,
            self.FNSeeds,
            self.pattern,
        )
    ...
    def ReadDcds(self, verbose=True, stride=1):
        ...
        for seedi in range(len(self.FNSeeds)):
            ...
            for di in range(len(self.simDirs)):
                ...
                print(
                    "TrajectoryAnalyzer simDirs get",
                    self.simDirs[di]
                    + "traj."
                    + self.molName
                    + self.pattern
    ...
    def Distance(self, indices):
        for seedi in range(len(self.FNSeeds)):
            self.distances[seedi] = md.compute_distances(
                self.trajectories[seedi], indices
            )
    ...
    def RMSD(self):
        for seedi in range(len(self.FNSeeds)):
            self.rmsds[seedi] = md.rmsd(
                self.trajectories[seedi], self.trajectories[seedi], 0
            )
            print("RMSD:")
    ...
    def SASA(self):
        for seedi in range(len(self.FNSeeds)):
            self.SASAs[seedi] = md.shrake_rupley(
                self.trajectories[seedi],
                probe_radius=0.14,
                mode="residue",
                get_mapping=False,
            )
    ...
    def Helicity(self):
        for seedi in range(len(self.FNSeeds)):
            self.dssps[seedi] = md.compute_dssp(
                self.trajectories[seedi], simplified=True
            )
