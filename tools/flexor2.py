...
class Flexor:
    ...
    def addWorld(
        ...
        cycleVisits = [
            len(x) - 1 if x[0].residue.name == "PRO" else len(x) for x in cycles
        ]
        if "side" in subset:
        ...
        for edge in self._edges:
            if (edge[0].index in selIx) and (edge[1].index in selIx):
                ...
                if jointType == "cart":
                    ...
                    fileOut.write(
                        "{:5d} {:5d} Cartesian #{} {}\n".format(
                            edge[0].index, edge[1].index, edge[0], edge[1]
                        )
                    )
                    ...
                    for subsetType in subset:
                        if (
                            ...
                            and not (self.check_cycle(edge[0], edge[1], cycleVisits))
                            or (
                                self.isProline(edge[0], edge[1], "N", "CD")
                                and subsetType == "side"
                            )
                        ):
                            if rolling == True:
                                fileOut = open(
                                    "{}.{}.flex".format(FNOut, fileCounter), "w"
                                )
                                fileNameIX.append(os.path.basename(fileOut.name))
                                fileCounter += 1
                            fileOut.write(
    ...
    def check_cycle(self, atom1, atom2, cycleList):
        ...
        for cycle in cycles:
            ...
            if (atom1 in cycle) and (atom2 in cycle):
                ...
                if (
                    cycleList[cycleIx] != -1
                ):  ## Meaning none of the bonds in this cycle have been welded
    ...
    def isProline(self, atom1, atom2, atom1Name, atom2Name):
        if (atom1.residue.name == "PRO" and atom2.residue.name == "PRO") and (
            atom1Name in [atom1.name, atom2.name]
            and atom2Name in [atom1.name, atom2.name]
        ):
            return True
        else:
            return False
