...
class Flexor:
    ...
    def create(self, range, subset, jointType, distanceCutoff=0, sasa_value=-1, onlyPrint=False):
        '''
        Parameters
        ----------
        range:  string
            ...
            if ((edge[0].index in selIx) and (edge[1].index in selIx)):
                if ("coils" in subset):
                    if (dssp[edge[0].residue.index] != "C" and dssp[edge[1].residue.index] != "C"):
                        continue
                if (jointType == "cart"):
                    print(f"{{ {edge[0].index}, {edge[1].index}, BondMobility::Mobility::Translation }},")
                    ...
                    for subsetType in subset:
                        if ((self.check_joint(edge[0], jointType, subsetType) and
                            ...
                            if jointType.capitalize() == 'Pin':
                                print(f"{{ {edge[0].index}, {edge[1].index}, BondMobility::Mobility::Torsion }},")
                            if jointType.capitalize() == 'Ball':
                                print(f"{{ {edge[0].index}, {edge[1].index}, BondMobility::Mobility::Ball }},")
