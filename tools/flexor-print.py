...
class Flexor:
    ...
    def create(
        self, range, subset, jointType, distanceCutoff=0, sasa_value=-1, onlyPrint=False
    ):
        """
        Parameters
        ----------
        ...
        subset: list of strings
            ...
            Options:
                ...
                ligand: DEPRECATED - Automatically uses n_bonds to det. joint types.
                        and check_cycle to determine whether to put a flexibility somewhere.
        ...
        for edge in self._edges:
            if (edge[0].index in selIx) and (edge[1].index in selIx):
                if "coils" in subset:
                    if (
                        dssp[edge[0].residue.index] != "C"
                        and dssp[edge[1].residue.index] != "C"
                    ):
                        continue
                if jointType == "cart":
                    print(
                        f"{{ {edge[0].index}, {edge[1].index}, BondMobility::Mobility::Translation }},"
                    )
                    ...
                    for subsetType in subset:
                        if (
                            self.check_joint(edge[0], jointType, subsetType)
                            and self.check_joint(edge[1], jointType, subsetType)
                        ) and not (self.check_cycle(edge[0], edge[1])):
                            if jointType.capitalize() == "Pin":
                                print(
                                    f"{{ {edge[0].index}, {edge[1].index}, BondMobility::Mobility::Torsion }},"
                                )
                            if jointType.capitalize() == "Ball":
                                print(
                                    f"{{ {edge[0].index}, {edge[1].index}, BondMobility::Mobility::Ball }},"
                                )
        ...
        if distanceCutoff > 0:
            ...
            nearbyAtoms = md.compute_neighbors(
                self._MDTrajObject, distanceCutoff, validJointAtoms
            )
    ...
    def check_joint(self, atom, jointType, subset):
        ...
        if atom.n_bonds > 1:
            if (
                (subset == "phi")
                and (atom.name in phi)
                and (atom.residue.name != "PRO")
            ):
                return True
...
mdtrajObj = md.load("alanine-dipeptide.rst7", top="alanine-dipeptide.prmtop")
flexorObj = Flexor(mdtrajObj)
