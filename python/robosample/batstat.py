...
class BATCorrelations:
    def __init__(self, prmtop_file, inpcrd_file, seed):
        ...
        (
            ...
            residue_ids,
            num_dihedrals,
        ) = self.build_dihedral_atom_groups(self.universe)
    ...
    def compute_dihedrals_from_dcd(self, dcd_files, start, stop=None, step=1):
        for dcd in dcd_files:
            ...
            (
                ...
                residue_ids,
                num_dihedrals,
            ) = self.build_dihedral_atom_groups(universe)
    ...
    def build_dihedral_atom_groups(self, universe):
        ...
        for res in universe.residues:
            ...
            for chi_name, chi_atoms in self.dihedral_sele[res.resname].items():
                chi_atom_ix = [
                    universe.select_atoms(f"resid {res.resid} and name {atom_name}").ix[
                        0
                    ]
                ...
                print(
                    f"Found dihedral {chi_name} in residue {res.resname} {res.resid} with atoms {chi_atoms}"
                )
        ...
        for bond in disulfide_bonds:
            ...
            for sg_atom_0, sg_atom_1 in sg_atom_pairs:
                sg0_selection = universe.select_atoms(
                    f"resid {sg_atom_0.resid} and name SG"
                )
                sg1_selection = universe.select_atoms(
                    f"resid {sg_atom_1.resid} and name SG"
                )
                cb_selection = universe.select_atoms(
                    f"resid {sg_atom_1.resid} and name CB"
                )
                ca_selection = universe.select_atoms(
                    f"resid {sg_atom_1.resid} and name CA"
                )

                atom_group = mda.AtomGroup(
                    [
                        sg0_selection.atoms[0],
                        sg1_selection.atoms[0],
                        cb_selection.atoms[0],
                        ca_selection.atoms[0],
                    ]
                )
    ...
    def dihedrals_to_pdb(self):
        with open("dihedrals.pdb", "w") as f:
            ...
            for aix, dihedral_type in zip(self.atom_indices, self.dihedral_types):
                ...
                hetatm.append(
                    f"HETATM{aix1 + 1:5d} {dihedral_type[:4]:4s} {atom1.resname:3s} {atom1.resid:4d}    {atom1.position[0]:8.3f}{atom1.position[1]:8.3f}{atom1.position[2]:8.3f}  1.00  0.00          {atom1.name:2s}"
                )
                hetatm.append(
                    f"HETATM{aix2 + 2:5d} {dihedral_type[:4]:4s} {atom2.resname:3s} {atom2.resid:4d}    {atom2.position[0]:8.3f}{atom2.position[1]:8.3f}{atom2.position[2]:8.3f}  1.00  0.00          {atom2.name:2s}"
                )
    ...
    def validate_covariance_matrix(self, covariance_matrix):
        ...
        try:
            _ = np.linalg.eigvalsh(covariance_matrix)
    ...
    def compute_correlations(self, null=False):
        ...
        for sublist in self.dihedral_values:
            ...
            with ThreadPoolExecutor() as executor:
                futures = [
                    executor.submit(self.circcorr, sublist, pair, null)
    ...
    def chose_decoy_bonds(self, ref_bonds, steps, seed):
        ...
        for i in range(steps):
            ...
            random_masses = self.compute_protein_cut_masses(
                random_bonds, len(ref_masses)
            )
