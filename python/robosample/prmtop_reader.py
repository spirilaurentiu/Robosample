...
def parse_prmtop_numpy(prmtop_file):
    ...
    for line in lines:
        ...
        chunks = [
            arr[i * i_length : (i + 1) * i_length].tobytes().decode("utf-8")
        ...
        if item_type.upper() == "A":
            ...
            raw_data[flag].extend(
                np.array([float(x.replace("D", "E")) for x in items], dtype=np.float64)
            )
        else:
            # fallback as string
            raw_data[flag].extend(items)
...
def has_nbfix_fast(
    nb_indices: np.ndarray, num_types: int, acoef: np.ndarray, bcoef: np.ndarray
) -> bool:
    nb_indices = np.array(nb_indices).reshape(num_types, num_types) - 1
