...
class MoleculePrototype:
    ...
    def _build_bonds_macrocycles(self) -> List[Tuple[int, int]]:
        ...
        for cycle in nx.cycle_basis(full_graph):
            ...
            for i in range(len(cycle)):
                ...
                dihedral_type = self._get_standardized_dihedral_type(self.molecule[atom_a], self.molecule[atom_b])
    ...
    def _build_bonds_bfs(self, root: pmd.Atom, forbidden_atom_type_pairs: Iterable[Tuple[str, str]] = [("SG", "SG")]) -> List[Tuple[int, int, bool]]:
        """
        ...
        # Count how many rigid bonds each atom is involved in to enforce the constraint that no atom can be involved in more than one rigid bond
        rigid_bonds_involved: Dict[int, int] = {}
        ...
        for bond in self.molecule.bonds:
            ...
            if 'ring' in dihedral_type:
                ...
                for idx in (parent_idx, child_idx):
                    ...
                    rigid_bonds_involved[idx] = rigid_bonds_involved.get(idx, 0) + 1
    ...
    def _get_standardized_dihedral_type(self, parent_atom: pmd.Atom, child_atom: pmd.Atom) -> str:
        """
