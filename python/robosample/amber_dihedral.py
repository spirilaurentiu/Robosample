...
def _extract_residue_indices(atoms: List[pmd.topologyobjects.Atom]) -> Tuple[int, int, int, int]:
    resids = (
        atoms[0].residue.idx,
        atoms[1].residue.idx,
        atoms[2].residue.idx,
    ...
    if not all(resid != -1 for resid in resids):
        raise ValueError(f"One or more atoms in the dihedral do not have a valid residue index: {resids}")
    return resids

def _safe_insert(dictionary: Dict[Tuple[str, str, str, str], str], key: Tuple[str, str, str, str], value: str):
    if key in dictionary:
        if dictionary[key] != value:
            raise ValueError(f"Conflict in dihedral definitions for {value}: {dictionary[key]} vs {key}")
    else:
        dictionary[key] = value

def _extract_atom_names(atoms: List[pmd.topologyobjects.Atom]) -> Tuple[str, str, str, str]:
...
class DihedralClassifier:
    ...
    def _classify_nucleic_acid(self, dihedral: pmd.topologyobjects.Dihedral) -> str | None:
    ...
    def _classify_protein(self, gparent: pmd.Atom, parent: pmd.Atom, child: pmd.Atom, gchild: pmd.Atom) -> str | None:
        ...
        if label == 'psi':
            if resids[0] == i and resids[1] == i and resids[2] == i and resids[3] == i+1:
                return label
            else:
                return None
        ...
        if label == 'omega':
            if resids[0] == i and resids[1] == i and resids[2] == i+1 and resids[3] == i+1:
                return label
            else:
                return None
    ...
    def _classify_glycan_linkage(self, dihedral: pmd.topologyobjects.Dihedral) -> str | None:
    ...
    def _classify_endocyclic_tau(self, dihedral: pmd.topologyobjects.Dihedral) -> str | None:
        """
        ...
        label = PYRANOSE_TAU_PATTERNS.get(atom_names) or PYRANOSE_TAU_PATTERNS.get(rev_names)
        if label:
            return f"pyranose-{label}"

        # 2. Check Furanose Patterns
        label = FURANOSE_TAU_PATTERNS.get(atom_names) or FURANOSE_TAU_PATTERNS.get(rev_names)
        if label:
    ...
    def _is_glycosidic_phi(self, dihedral: pmd.topologyobjects.Dihedral) -> bool:
        ...
        def check(atoms: List[pmd.topologyobjects.Atom]) -> bool:
            ...
            if not any(names[2].startswith(e) for e in ("O", "N", "S")): return False
            ...
            if cx is None or not (1 <= cx <= 9): return False
    ...
    def _is_glycosidic_psi(self, dihedral: pmd.topologyobjects.Dihedral) -> bool:
        ...
        def check(atoms: List[pmd.topologyobjects.Atom]) -> bool:
            ...
            if resids[0] == resids[1]: return False
            if not (resids[1] == resids[2] == resids[3]): return False

            # 1. Anomeric Carbon (Donor i)
            if names[0] not in ("C1", "C2"): return False

            # 2. Linkage Atom (Acceptor j)
            if not any(names[1].startswith(e) for e in ("O", "N", "S")): return False
            ...
            if nx is None or not (1 <= nx <= 9): return False
            ...
            if ox_idx is not None and ox_idx != cx: return False
    ...
    def _is_glycosidic_omega(self, dihedral: pmd.topologyobjects.Dihedral) -> bool:
        ...
        def check(atoms) -> bool:
            ...
            if not any(p.residue.idx != linkage_atom.residue.idx for p in linkage_atom.bond_partners):
                return False
        ...
        def check(atoms, names) -> str | None:
            ...
            if a4.startswith('O') and a4 not in ('O4', 'O5'):
                ...
                if all(idx is not None for idx in c_indices):
                    ...
                    is_bridge = any(p.residue.idx != atoms[3].residue.idx for p in atoms[3].bond_partners)
                    if not is_bridge:
                        return "glycosidic-chi-exocyclic"

            # 2. Anomeric Substituents (C1-O1-X)
            if a2 == "C1" and a1 in ("O5", "O4") and a3.startswith('O') and a3 not in ("O4", "O5"):
                return "glycosidic-chi-anomeric"

            # 3. Sulfates/Phosphates
            if _extract_carbon_index(a1) is not None and a2.startswith('O'):
                if 'S' in a3: return "glycosidic-chi-sulfate"
                if 'P' in a3: return "glycosidic-chi-phosphate"
    ...
    def _classify_substituent(self, dihedral: pmd.topologyobjects.Dihedral) -> str | None:
        """
            ...
            if (at2.element_name in ['N', 'O']) and at3.element_name == 'C':
                ...
                if 'O' in neighbors:
                    ...
                    if at_list[3].element_name == 'O':
                        return "glycan-chi-subst-amide" if at2.element_name == 'N' else "glycan-chi-subst-ester"
